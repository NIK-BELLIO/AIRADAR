...
async function vsAutoAiChat(prompt, opts) {
  ...
  for (let pass = 0; pass < 3; pass++) {
    ...
    // proxied last resort within each pass
    for (const useJson of (wantJson ? [true, false] : [false])) {
      try { return await postChat("https://text.pollinations.ai/openai", "openai", useJson, true); }
      catch (e) { /* next */ }
    }
    if (pass < 2) await sleep(1200 * (pass + 1));   // back off, then retry
  }
...
function vsAssembleFromSections(data) {
  ...
  const wantImages = true;
  if (wantImages) {
    vsAutoGenerateBackgrounds(data);
  }
}

// Fetch a real stock video clip from Pexels. Returns a ready, CORS-clean
// <video> element, or null on any failure.
async function vsFetchPexelsClip(query, key, aspect) {
  ...
  // gather candidate mp4 files, preferring ~720p (loads fast, still crisp)
  const cands = [];
  for (const v of vids) {
    for (const f of (v.video_files || [])) {
      if (/mp4/i.test(f.file_type || "") && f.link) cands.push(f);
    }
  }
  cands.sort((a, b) => Math.abs((a.height || 0) - 720) - Math.abs((b.height || 0) - 720));
  // try the best few in turn so one slow/blocked file doesn't kill footage
  for (const file of cands.slice(0, 3)) {
    const vid = await new Promise((resolve) => {
      const el = document.createElement("video");
      el.crossOrigin = "anonymous";
      el.muted = true; el.loop = true; el.playsInline = true; el.preload = "auto";
      let done = false;
      const timer = setTimeout(() => { if (!done) { done = true; resolve(null); } }, 9000);
      el.onloadeddata = () => {
...
async function vsFetchPexelsPhoto(query, key, aspect) {
  const orient = aspect === "16:9" ? "landscape" : aspect === "1:1" ? "square" : "portrait";
...
async function vsAutoGenerateBackgrounds(data) {
  ...
  for (let i = 0; i < slides.length; i++) {
    const s = slides[i];
    if (s.mediaEl) continue;   // already has footage — leave it
    // footage now covers EVERY scene (intro, text, infographic, outro)
    const subject = (s.settings && (s.settings["#vsNewsHeadline"] || s.settings["#vsHeadline"]))
                    || s.headline || s._caption || data.title || "abstract concept";
    vsAutoStatus(state.lang === "fa"
    ...
    // ── REAL footage first (Pexels), if enabled. Uses the owner's embedded
    //    key when present so end-users never need to enter anything. ──
    ...
    if (pexelsOn && pexelsKey) {
      vsAutoStatus(state.lang === "fa" ? `در حال یافتن فوتیج واقعی صحنه ${i + 1}…`
                                       : `Finding real footage for scene ${i + 1}…`);
      // Build a RELEVANT query: prefer the real place (city), else a couple of
      // meaningful words from the headline/subject — never the generic caption.
      const topicWords = String(s.headline || subject || data.title || "")
        .replace(/[^\w\s]/g, " ").split(/\s+/).filter(w => w.length > 3).slice(0, 3).join(" ");
      const query = firstPlace ? (firstPlace + " city") : (topicWords || "cinematic background");
      try {
        const aspect = vsVal("#vsAspect", "9:16");
        let media = await vsFetchPexelsClip(query, pexelsKey, aspect);
        if (!media) media = await vsFetchPexelsPhoto(query, pexelsKey, aspect);
        if (media) {
          s.mediaEl = media; s.isVideo = (media.tagName === "VIDEO");
          s.ready = true; s.url = media.currentSrc || media.src;
          s.settings = s.settings || {};
          s.settings["#vsMotion"] = camMoves[camIdx++ % camMoves.length];
          made++; renderSlideList();
          if (vstudio.activeSlide === i) drawStudioFrame(vstudio.position || 0);
          continue;   // got real footage — skip AI image for this scene
        }
    ...
    try {
      const img = await vsGenerateImage(
        subjectCtx + ", " + palette + " color palette, editorial photography", null);
...
function vsExtractItemsLocal(text) {
  ...
  const add = (city, st) => {
    ...
    seen.add(key); found.push(name);
  };
  const cityPat = "([A-Z][A-Za-z.'’-]+(?:\\s+[A-Z][A-Za-z.'’-]+){0,2})";
  // City, ST
  let m, re = new RegExp(cityPat + ",\\s*([A-Z]{2})\\b", "g");
...
function vsBasicCityData(name, excerpt, topic, source) {
  ...
  // prefer sentences that actually mention the place
  const mentions = all.filter(s => new RegExp(short, "i").test(s));
  const sents = (mentions.length ? mentions : all);
  const clip = (s, n) => s && s.length > n ? s.slice(0, n).trim() + "…" : s;
  const h1 = clip(sents[0], 90) || (short + " is worth a closer look.");
  const h2 = clip(sents[1] || sents[0], 90) || ("What makes " + short + " stand out.");
  return {
    title: name, subtitle: topic || "", kicker: "SPOTLIGHT",
    source: source || "AI Radar", palette: "gold",
    intro: { main: short, sub: topic || "" },
    sections: [
      { type: "text", caption: "SPOTLIGHT", headline: h1, style: "title-center" },
      { type: "text", caption: "WHY IT MATTERS", headline: h2, style: "bold-statement" }
    ],
    outro: { main: short, sub: "AI Radar" }
  };
}
...
async function vsBuildBatchFromArticle(text, tone, lenChoice) {
  ...
  // 1) EXTRACT just the LIST OF NAMES the article profiles. Keeping this
  //    JSON tiny makes the free model far more reliable than asking it to
  //    dump every item's facts at once (which often truncates/breaks).
  const exPrompt =
`Read the ARTICLE and list EVERY distinct item it profiles (e.g. cities, products, companies, people), in the order they appear, using the name exactly as written.
...
ARTICLE: """${String(text).slice(0, 12000)}"""`;
  ...
  let ex = null, names = [];
  for (let attempt = 0; attempt < 2 && names.length < 2; attempt++) {
    const p = attempt === 0 ? exPrompt
      : exPrompt + "\n\nThe article DOES profile several distinct items — list ALL of them. Output JSON only.";
    try { ex = vsParseAiJson(await vsAutoAiChat(p)); } catch (e) { ex = null; }
    names = resolveItems(ex)
  ...
  if (names.length < 2) {
    // AI extraction failed (often a transient outage) — fall back to a
    // local, no-AI scan of the article for "City, ST" items.
    const local = vsExtractItemsLocal(text);
    if (local.length >= 2) { names = local; ex = ex || {}; }
  }
  const usedLocalNames = !(ex && ex.items) && names.length >= 2 && !(ex && ex.topic);
  if (names.length < 2) {
  ...
  const source = (ex && ex.source) || "AI Radar — Newsroom";
  // 4-scene videos: intro + 2 content + outro (user wants ~4 slides each)
  ...
  // 2) SCRIPT each item from its OWN slice of the article (so the facts for
  //    that exact item are always present — fixes the "no data" scenes).
  //    If the AI is down, skip it after a couple of misses and build locally.
  let aiFailStreak = usedLocalNames ? 2 : 0;
...
Base it on the SOURCE TEXT about ${name} below. Prefer its real figures, prices, ranks and reasons.
If the SOURCE TEXT is thin, still write an engaging, ACCURATE general profile of ${name} relevant to "${topic}" — but never fabricate exact statistics.
...
2. INFOGRAPHIC only if real numbers for ${name} exist in the SOURCE TEXT; otherwise use TEXT styles. Never two infographics in a row.
3. title MUST be exactly "${name}". Keep every line concrete, specific and free of meta-commentary.
SOURCE TEXT about ${name}: """${excerpt}"""`;
    ...
    if (data && Array.isArray(data.sections) && data.sections.length) {
      ...
      aiFailStreak++;
      data = vsBasicCityData(name, excerpt, topic, source);   // guaranteed local build
    }
    if (data && Array.isArray(data.sections) && data.sections.length) {
  ...
  vsAutoStatus((usedLocalNames
      ? (fa ? "هوش مصنوعی شلوغ بود — ویدئوها مستقیم از متن مقاله ساخته شدند. "
            : "AI was busy — videos were built straight from the article text. ")
      : "") +
    (fa ? `${vstudio.batchVideos.length} ویدئو آماده شد. روی هر کدام بزن یا ⬇ را برای دانلود بزن.`
        : `${vstudio.batchVideos.length} videos ready. Click any to open, or ⬇ to download.`));
...
async function vsLoadBatchVideo(i) {
  ...
  const fa = state.lang === "fa";
  vsAssembleFromSections(v.data);          // build this item's slides
  vsRenderBatchList();
  // footage is now ALWAYS auto-generated (real Pexels clips → AI image fallback)
  if (!v._footaged) {
    vsAutoStatus(fa ? `در حال ساخت فوتیج برای ${v.name}…` : `Finding footage for ${v.name}…`);
    try { await vsAutoGenerateBackgrounds(v.data); v._footaged = true; } catch (e) {}
  }
...
async function vsExportOne(i) {
  const v = vstudio.batchVideos[i];
  if (!v || vstudio.rendering || vstudio._batchExporting) return;
  const fa = state.lang === "fa";
...
function vsRenderBatchList() {
  ...
  let html = `<div class="vs-batch-head"><span>${fa ? "ویدئوها" : "Videos"} (${vids.length})</span>` +
    `<button type="button" id="vsBatchExportAll" class="vs-batch-dlall">${fa ? "⬇ دانلود همه" : "⬇ Download all"}</button></div>`;
  vids.forEach((v, i) => {
...
async function vsExportAllBatch() {
  const fa = state.lang === "fa";
  const vids = vstudio.batchVideos || [];
  if (!vids.length || vstudio.rendering || vstudio._batchExporting) return;
  const proceed = window.confirm(fa
    ? `همهٔ ${vids.length} ویدئو پشت‌سرهم ساخته و دانلود می‌شوند. ممکن است چند دقیقه طول بکشد و باید این تب باز بماند. ادامه؟`
    : `This renders and downloads all ${vids.length} videos one by one. It can take several minutes — keep this tab open. Continue?`);
  if (!proceed) return;
  vstudio._batchExporting = true;
  vstudio._batchCancel = false;
  ...
  for (let i = 0; i < vids.length; i++) {
    ...
    try {
      await vsLoadBatchVideo(i);                 // assemble + footage for this one
      vstudio._exportName = vids[i].name + " - AI Radar";
      await exportStudioVideo();                 // record + download
    } catch (e) { /* skip a failed one, continue */ }
    vstudio._exportName = null;
  }
  vstudio._batchExporting = false;
  vsBatchProgress(false);
  if (btn) { btn.disabled = false; btn.textContent = fa ? "⬇ دانلود همه" : "⬇ Download all"; }
  vsAutoStatus(fa ? `همهٔ ${vids.length} ویدئو دانلود شد.` : `All ${vids.length} videos downloaded.`);
}
...
`;
  ...
  async function send() {
    ...
    try {
      const reply = await vsAutoAiChat(prompt, { json: false });
