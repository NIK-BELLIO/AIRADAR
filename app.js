...
function vsAssembleFromSections(data, skipFootage) {
  ...
  sections.forEach((sec, i) => {
    ...
    if (sec.type === "infographic" && Array.isArray(sec.stats) && sec.stats.length) {
      ...
      // Text / narrative slide — pick a style that FITS the text length so the
      // whole headline is always visible, and vary it for contrast.
      const hl = String(sec.headline || sec.title || "").trim();
      const len = hl.length;
      let pool;
      if (len <= 34)       pool = ["bold-statement", "title-center", "quote", "title-left"];
      else if (len <= 72)  pool = ["title-center", "title-left", "quote", "caption"];
      else                 pool = ["title-center", "caption", "title-left"];   // long → roomy auto-fit
      ...
      set["#vsNewsHeadline"] = hl.slice(0, 200);
...
• Every scene pairs a concrete figure with its IMPLICATION for the client (e.g. "a $540K median means a solo buyer needs roughly $X income to qualify").
