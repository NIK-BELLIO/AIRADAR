...
try { console.log("%cAI Radar Studio build v340", "color:#d8b76a;font-weight:bold"); } catch(e){}
...
async function vsGenerateNarration(data, voice) {
  try {
    ...
    if (script.length > 550) script = script.slice(0, 550);   // keep the URL short & reliable

    const v = voice || ((document.querySelector("#vsVoice") || {}).value) || "nova";
    const base = "https://text.pollinations.ai/" + encodeURIComponent(script) +
      "?model=openai-audio&voice=" + encodeURIComponent(v);
    // Try plain first (most CORS-friendly); then with the key as a query token.
    const urls = [base];
    if (VS_POLLINATIONS_KEY) urls.push(base + "&token=" + encodeURIComponent(VS_POLLINATIONS_KEY));
    let blob = null;
    for (const u of urls) {
      try {
        const ctrl = new AbortController();
...
4. INFOGRAPHIC only where a real/realistic number fits; never two in a row. Keep stat labels ≤14 chars and values ≤9 chars so they never overflow.
