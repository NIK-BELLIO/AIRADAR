...
const i18n = {
  en: {
    ...
    aiStudioText: "A dedicated AI Studio for Iranian creators: text-to-image, image-to-video with camera motion, Rial payment via Zarinpal, and simple email sign-in. Opens in its own app.",
    aiStudioChip1: "Text → image",
    aiStudioChip2: "Image → video",
    aiStudioChip3: "Rial payment",
    aiStudioChip4: "Email sign-in, no password",
    aiStudioCta: "Open AI Studio ↗",
  ...
  fa: {
    ...
    aiStudioText: "استودیوی اختصاصی هوش مصنوعی برای سازندگان ایرانی: متن به تصویر، تصویر به ویدیو با کنترل حرکت دوربین، پرداخت ریالی با زرین‌پال و ورود ساده با ایمیل. در اپ جداگانه‌ی خودش باز می‌شود.",
    aiStudioChip1: "متن ← تصویر",
    aiStudioChip2: "تصویر ← ویدیو",
    aiStudioChip3: "پرداخت ریالی",
    aiStudioChip4: "ورود با ایمیل، بدون پسورد",
    aiStudioCta: "باز کردن استودیوی هوش مصنوعی ↗",
...
const state = {
  ...
  uploadedAssets: [],
  // Start fresh after refresh; load projects only through an explicit action.
  slides: []
};
...
function vsRemoveLegacyVoiceControls() {
  const toggle = document.getElementById("vsVoiceover");
  const label = toggle && toggle.closest("label");
  if (label) label.remove();
  document.getElementById("vsVoiceRow")?.remove();
  document.getElementById("vsVoiceoverLbl")?.remove();
}
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", vsRemoveLegacyVoiceControls, { once: true });
} else {
  vsRemoveLegacyVoiceControls();
}
...
const vstudio = {
  ...
  musicUrl: null,
  musicEl: null,
  narrationEl: null, narrationUrl: null,
  logoUrl: null,
...
function vsAssembleStory({ title, subtitle, stats, points, kicker, outroMain, captions }) {
  ...
  vstudio.slides.push({
    ...
    introMotion: "blur", headline: "", duration: 3,
    settings: cleanSet(),
...
async function vsAutoAiChat(prompt, opts) {
  ...
  // Optional PREMIUM model via the user's own OpenRouter key → a big quality
  // jump (GPT-4o / Claude Sonnet / etc.). Tried first; falls back to free.
  async function postOpenRouter(key, model, useJson) {
    const body = {
      model: model,
      messages: [{ role: "user", content: prompt }],
      temperature: 0.6
    ...
    if (useJson) body.response_format = { type: "json_object" };
    const ctrl = new AbortController();
    const tm = setTimeout(() => ctrl.abort(), Math.max(timeoutMs, 30000));
    let resp;
    try {
      resp = await fetch(VS_USER_API_BASE, {
        method: "POST",
  ...
  const userKey = (((document.querySelector("#vsApiKey") || {}).value || "").trim()) || VS_USER_API_KEY || "";
  const userModel = ((document.querySelector("#vsApiModel") || {}).value || "openai/gpt-4o").trim();
  if (userKey && !opts.fast) {
    for (const useJson of (wantJson ? [true, false] : [false])) {
      if (vstudio._batchCancel) return null;
      try { return await postOpenRouter(userKey, userModel, useJson); }
      catch (e) { /* premium failed → fall back to the free models below */ }
    }
  }
...
// Reject single-video scripts that expose the production process or invent
// numbers not present in the SOURCE. Mirrors vsIsGroundedAnalystScript (used
// for per-item batch scripts) but tuned for a general article/topic script:
// no city-specific heuristics, and it only enforces number-grounding when a
// real SOURCE (fetched article / pasted text) was actually supplied.
function vsIsGroundedScript(data, excerpt) {
  if (!data || !Array.isArray(data.sections) || !data.sections.length) return false;
  const raw = JSON.stringify(data);
  const lower = raw.toLowerCase();
  const forbidden = /no data|not provided|verify locally|verify (the )?numbers|article footnote|source text|source excerpt|as an ai|i cannot|i can.?t\s/;
  if (forbidden.test(lower)) return false;

  const source = String(excerpt || "").toLowerCase().replace(/,/g, "");
  if (source.length < 200) return true;   // topic-only / too little to ground against

  for (const section of data.sections) {
    if (String(section.type).toLowerCase() !== "infographic") continue;
    const stats = Array.isArray(section.stats) ? section.stats : [];
    for (const stat of stats) {
      const value = String(stat && stat.value || "").toLowerCase().replace(/,/g, "");
      const nums = value.match(/\d+(?:\.\d+)?/g) || [];
      if (nums.length && !nums.some(n => source.includes(n))) return false;   // invented figure
    }
  }
  return true;
}
...
async function buildAutoVideo(useAI) {
  ...
  // or pasted text), or is this "smart" mode with just a bare topic idea?
...
SOURCE: """${text.slice(0, 9000)}"""`;
  ...
  for (let attempt = 0; attempt < 2 && !data; attempt++) {
    ...
    try {
      const raw = await vsAutoAiChat(buildScriptPrompt(attempt > 0));
...
async function vsFetchArticle(url) {
  ...
  const cleanupText = (t) => {
    return t
      ...
      // Preserve Markdown line boundaries so ARTICLE TITLE / SECTIONS remain
      // deterministic and cannot absorb the article body or footer links.
      .replace(/[^\S\r\n]+/g, " ")
      .replace(/\n{3,}/g, "\n\n")
      .trim();
  };
  ...
  const focusArticle = (t) => {
    ...
    // Stop at recommendation/footer modules. Their linked headlines contain
    // unrelated cities and previously inflated a 10-market article to 16.
    const endMarkers = [
...
function vsAssembleFromSections(data, skipFootage) {
  ...
  // Show the real outlet as "by Publisher". Drop the old AI-Radar placeholder.
  let srcRaw = (data.source && String(data.source).trim()) ? String(data.source).trim().slice(0, 50) : "";
  if (/^ai\s*radar/i.test(srcRaw)) srcRaw = "";
  srcRaw = srcRaw.replace(/^by\s+/i, "").trim();
  ...
  sections.forEach((sec, i) => {
    ...
    // Parse infographic stats robustly — the AI sometimes uses alternate keys
    // (name/amount/figure…) or leaves blanks, which produced empty cards.
    const clampLabel = (s) => {
      s = String(s || "").replace(/\s+/g, " ").trim();
      if (s.length <= 32) return s;                    // graphics auto-fit labels
      const t = s.slice(0, 32), sp = t.lastIndexOf(" ");
      return (sp > 16 ? t.slice(0, sp) : t).trim();   // cut on a word boundary
    };
    let parsedStats = [];
    if (sec.type === "infographic" && Array.isArray(sec.stats)) {
      parsedStats = sec.stats.map(s => {
      ...
      }).filter(s => s && (s.label || s.value)).slice(0, 5);
    }

    if (sec.type === "infographic" && parsedStats.length >= 1) {
      sec.stats = parsedStats;
      const n = sec.stats.length;
      const maxLabel = Math.max(0, ...sec.stats.map(s => s.label.length));
      // a value is "tight-safe" only if it's short AND numeric-ish ($, %, digits)
      const isShortNum = (v) => { const s = String(v).trim(); return s.length <= 7 && /[\d%$]/.test(s); };
      const allShort = sec.stats.every(s => isShortNum(s.value));
      const longText = maxLabel > 14 || !allShort;   // anything else would overflow donut/pills
      ...
      // chosen to FIT the text length so the whole line stays visible
      let pool;
...
// ── Default background music: render a calm cinematic ambient pad in-browser
//    (no external file, royalty-free) and hand back a looping <audio> element ──
function vsAudioBufferToWav(buffer) {
  const numCh = buffer.numberOfChannels, len = buffer.length;
  const sr = buffer.sampleRate, bytesPerSample = 2;
...
async function vsEnsureDefaultMusic(data) {
  try {
    ...
    comp.attack.value = 0.02; comp.release.value = 0.3; comp.connect(master);

    // sidechain bus — the melodic/pad layers route through this and get "pumped"
    // (ducked) on every kick, the signature feel of modern cinematic/ad music
    const sideGain = ctx.createGain(); sideGain.gain.value = 1.0; sideGain.connect(comp);

    // ── BASS layer: a deep sub + fifth drone with a slow heartbeat pulse ──
    const bassLP = ctx.createBiquadFilter(); bassLP.type = "lowpass"; bassLP.frequency.value = 260; bassLP.connect(sideGain);
    const bassPulse = ctx.createGain(); bassPulse.gain.value = 0.5; bassPulse.connect(bassLP);
    const pulse = ctx.createOscillator(); pulse.type = "sine"; pulse.frequency.value = 0.5; // ~30 bpm swell
    const pulseAmt = ctx.createGain(); pulseAmt.gain.value = 0.32;
    pulse.connect(pulseAmt); pulseAmt.connect(bassPulse.gain); pulse.start(0); pulse.stop(dur);
    [
      { f: 49.0, t: "sine", g: 0.9 },   // G1 sub
      { f: 65.41, t: "sine", g: 0.7 },  // C2
      { f: 98.0, t: "triangle", g: 0.35 } // G2 body
    ].forEach(n => {
      const o = ctx.createOscillator(); o.type = n.t; o.frequency.value = n.f;
      const g = ctx.createGain(); g.gain.setValueAtTime(0.0001, 0);
      g.gain.linearRampToValueAtTime(n.g, 3); g.gain.setValueAtTime(n.g, dur - 5);
      g.gain.linearRampToValueAtTime(0.0001, dur);
      o.connect(g); g.connect(bassPulse); o.start(0); o.stop(dur);
    });

    // ── PAD layer: a moody Cm9 chord, filtered + slow tremolo (cinematic) ──
    const padLP = ctx.createBiquadFilter(); padLP.type = "lowpass"; padLP.frequency.value = 1600; padLP.Q.value = 0.7; padLP.connect(sideGain);
    ...
    chord.forEach((f, i) => {
      [0, 2.2].forEach(det => {
        ...
        const g = ctx.createGain(); const peak = 0.12 / (1 + i * 0.18);
        const t0 = 2 + i * 0.5;
        g.gain.setValueAtTime(0.0001, 0);
        g.gain.linearRampToValueAtTime(peak, t0 + 2.5);
        g.gain.setValueAtTime(peak, dur - 4.5);
        g.gain.linearRampToValueAtTime(0.0001, dur);
        const lfo = ctx.createOscillator(); lfo.frequency.value = 0.07 + i * 0.015;
        const lg = ctx.createGain(); lg.gain.value = peak * 0.45;
        lfo.connect(lg); lg.connect(g.gain); lfo.start(0); lfo.stop(dur);
        o.connect(g); g.connect(padLP); o.start(0); o.stop(dur);
      });
    });

    // ── faint high shimmer for sparkle ──
    const shim = ctx.createOscillator(); shim.type = "sine"; shim.frequency.value = 1046.5;
    const sg = ctx.createGain(); sg.gain.value = 0.0;
    sg.gain.setValueAtTime(0.0001, 0); sg.gain.linearRampToValueAtTime(0.02, 6);
    sg.gain.setValueAtTime(0.02, dur - 5); sg.gain.linearRampToValueAtTime(0.0001, dur);
    shim.connect(sg); sg.connect(sideGain); shim.start(0); shim.stop(dur);
    ...
    const noiseAt = (t, dur2, hp, gain) => {
      ...
      s.connect(f); f.connect(g); g.connect(drumBus); s.start(t); s.stop(t + dur2 + 0.02);
    };
    const kickAt = (t, gain) => {
      ...
      o.connect(g); g.connect(drumBus); o.start(t); o.stop(t + 0.26);
    };
    const bassRoots = [49.0, 51.91, 43.65, 46.25];
    const bassAt = (t, freq) => {
      const o = ctx.createOscillator(); o.type = "triangle"; o.frequency.value = freq;
      const g = ctx.createGain();
      g.gain.setValueAtTime(0.0001, t);
      g.gain.linearRampToValueAtTime(0.5, t + 0.02);
      g.gain.exponentialRampToValueAtTime(0.0001, t + beat * 0.5);
...
async function vsGenerateNarrationLegacy(data, voice) {
  try {
    ...
    (data.sections || []).forEach(sec => {
      const line = sec.headline || sec.title || "";
      if (line) parts.push(String(line).replace(/[#*_`]/g, ""));
    });
    if (data.outro && data.outro.main) parts.push(String(data.outro.main).replace(/[#*_`]/g, ""));
    let script = parts.filter(Boolean).join(". ").replace(/\.\.+/g, ".").trim();
    if (!script) return null;
    // StreamElements caps length, so keep narration tight (it's a hook, not an essay)
    if (script.length > 480) script = script.slice(0, 480);

    const v = voice || ((document.querySelector("#vsVoice") || {}).value) || "nova";
    // Map the friendly voice names to Amazon-Polly voices (StreamElements) and
    // to Pollinations voices for the fallback.
    const seMap = { nova:"Joanna", shimmer:"Amy", alloy:"Salli", onyx:"Matthew", echo:"Joey", fable:"Brian" };
    const seVoice = seMap[v] || "Joanna";
    const urls = [
      // 1) StreamElements — free, no key, CORS-friendly, very reliable
    ...
    for (const u of urls) {
      try {
        ...
        let resp;
        try { resp = await fetch(u, { signal: ctrl.signal }); }
        finally { clearTimeout(tm); }
        if (!resp.ok) continue;
        const b = await resp.blob();
        if (b && b.size > 800) { blob = b; break; }
    ...
    // are captured reliably by the export recorder (element capture can go silent).
    let buffer = null;
    try {
      if (!vstudio._playCtx) vstudio._playCtx = new (window.AudioContext || window.webkitAudioContext)();
      const arr = await blob.arrayBuffer();
      buffer = await vstudio._playCtx.decodeAudioData(arr.slice(0));
    } catch (e) { buffer = null; }
...
async function vsAutoGenerateBackgrounds(data) {
  ...
  // If the article is about housing/real estate, theme content footage to it so
  // clips stay on-topic instead of matching stray headline keywords.
  const themeStr = String((data && (data._topic || data.subtitle || data.title)) || "").toLowerCase();
  const isHousing = /home|house|mortgage|buy|rent|real estate|property|housing|owner|loan|down payment|afford/.test(themeStr);
  const houseViews = ["house for sale", "suburban home", "house keys handover", "real estate sign",
  ...
  const genOne = async (s, i) => {
    ...
    if (pexelsOn && pexelsKey) {
      try {
        let media = await tryPexels(query, i);
        if (!media && fallbackQuery && fallbackQuery !== query && !vstudio._batchCancel) {
          media = await tryPexels(fallbackQuery, i + 3);
        }
...
// analyst's report, never like an AI apologizing for missing data.
function vsIsGroundedAnalystScript(data, excerpt) {
  if (!data || !Array.isArray(data.sections) || !data.sections.length) return false;
  const raw = JSON.stringify(data);
  const lower = raw.toLowerCase();
  const forbidden = /no data|not provided|verify locally|verify (the )?numbers|article footnote|source text|source excerpt|listed among (the )?10|one of (the )?10|included in (the )?list|rising label|watchlist signal|article proof|confirmed inclusion|market count|#10|rank(?:ed|ing)?\s*#?10/;
  if (forbidden.test(lower)) return false;
  ...
  for (const section of data.sections) {
    ...
    for (const stat of stats) {
      const value = String(stat && stat.value || "").toLowerCase().replace(/,/g, "");
      const nums = value.match(/\d+(?:\.\d+)?/g) || [];
      if (!nums.length || nums.some(n => n === "10" || n === "2026" || !source.includes(n))) return false;
    }
  }
  return true;
}
...
function vsCleanItemNames(names, fullText) {
  ...
  const JUNKLEAD = /^(mortgage|expo|conference|summit|webinar|event|events|register|sponsored|presented|annual|virtual|free|live|women|men|single|parent|parents|homeownership|market|markets|report|study|exclusive|best|worst|top|most|least|cheapest|affordable|safest|nicest|coolest|review|guide|ranking|list)\s+/i;
  // strip ad/section/demographic words glued before a city ("Mortgage Expo Chandler, AZ" → "Chandler, AZ")
  const stripLead = (n) => {
    let s = String(n).trim();
    let prev;
...
function vsExtractItemsLocal(text) {
  ...
  // ad / section / demographic words that get glued in front of a city name
  const JUNKLEAD = /^(mortgage|expo|conference|summit|webinar|event|events|register|sponsored|presented|annual|virtual|free|live|women|men|single|parent|parents|homeownership|market|markets|report|study|exclusive|best|worst|top|most|least|cheapest|affordable|safest|nicest|coolest|review|guide|ranking|list)$/i;
  ...
  const add = (city, st, arr) => {
    ...
    // strip leading connective + ad/section/demographic words
    // ("Unlike Tulsa" → "Tulsa", "Mortgage Expo Chandler" → "Chandler", "Women New Orleans" → "New Orleans")
    let parts = city.split(/\s+/);
    while (parts.length > 1 && (STOP.test(parts[0]) || JUNKLEAD.test(parts[0]))) parts.shift();
    city = parts.join(" ");
    if (!city || STOP.test(city) || JUNKLEAD.test(city)) return;
    const name = city + ", " + st;
  ...
  // A 1-2 item PASS-1 match (e.g. a single-city heading like "## Hartford,
  // Connecticut") was previously discarded here: it got marked "seen" but,
  // since numbered.length < 3, only `found` was returned — and PASS 2 skips
  // anything already "seen", so that city silently vanished. Merge PASS 1's
  // matches back in so single-city headings and 2-city ones ("Buffalo, New
  // York and Syracuse, New York") aren't lost.
  const merged = numbered.slice();
  found.forEach(n => { if (!merged.some(x => x.toLowerCase() === n.toLowerCase())) merged.push(n); });
  return merged;
}
...
function vsBasicCityData(name, excerpt, topic, source, count) {
  ...
  const caps = ["AFFORDABILITY", "PRICE TREND", "BUYING POWER", "MONTHLY COST", "MARKET OUTLOOK", "THE NUMBERS", "BOTTOM LINE"];
  // advisor-toned angle templates (used only when live data is unavailable)
  const angles = [
    short + "'s entry prices still sit within reach for prepared buyers.",
    "For " + short + " clients, monthly costs hinge on rate and down payment.",
    short + " rewards buyers who lock budgets before they shop.",
    "In " + short + ", inventory and timing decide who wins the deal.",
    short + "'s cost of living leaves real room in a household budget.",
    "A steady " + short + " market favours buyers planning 3-5 years out.",
    short + " is one to watch — value today, upside for the patient."
  ];
  const sections = [];
  for (let i = 0; i < n; i++) {
  ...
  return new Promise(resolve => {
    ...
    ov.innerHTML =
      `<div class="vs-pick">
        ...
        <div class="foot">
          <button class="cancel" id="vsPickCancel">${fa ? "انصراف" : "Cancel"}</button>
          <button class="go" id="vsPickGo"></button>
        </div>
      </div>`;
    document.body.appendChild(ov);
    const list = ov.querySelector("#vsPickList");
    names.forEach((n, i) => {
      const lab = document.createElement("label");
      lab.innerHTML = `<input type="checkbox" checked data-i="${i}"><span>${String(n).replace(/</g,"&lt;")}</span>`;
      list.appendChild(lab);
    });
    const boxes = () => Array.from(ov.querySelectorAll("#vsPickList input"));
    const goBtn = ov.querySelector("#vsPickGo");
    const upd = () => {
      const n = boxes().filter(b => b.checked).length;
      goBtn.textContent = (fa ? "بساز" : "Generate") + " (" + n + ")";
      goBtn.disabled = n === 0; goBtn.style.opacity = n === 0 ? ".5" : "1";
    };
    list.addEventListener("change", upd); upd();
    ov.querySelector("#vsPickAll").onclick = () => { boxes().forEach(b => b.checked = true); upd(); };
    ov.querySelector("#vsPickNone").onclick = () => { boxes().forEach(b => b.checked = false); upd(); };
    const close = (val) => { try { ov.remove(); } catch (e) {} resolve(val); };
    ov.querySelector("#vsPickCancel").onclick = () => close(null);
    ov.querySelector("#vsPickGo").onclick = () => {
      const chosen = boxes().filter(b => b.checked).map(b => names[+b.getAttribute("data-i")]);
      close(chosen.length ? chosen : null);
    };
...
async function vsBuildBatchFromArticle(text, tone, lenChoice) {
  ...
  // Prefer the article's own ordered section headings. This preserves the
  // published order and splits combined headings such as Buffalo + Syracuse.
  const sectionLine = String(text).match(/ARTICLE SECTIONS:\s*([^\n]+)/i);
  let authoritativeNames = false;
  if (sectionLine) {
    const ordered = [];
    sectionLine[1].split("|").map(s => s.trim()).forEach(heading => {
      if (/\b(?:City|Suburbs)$/i.test(heading) && !ordered.includes(heading)) ordered.push(heading);
      vsExtractItemsLocal("## " + heading).forEach(n => {
        if (!ordered.some(x => x.toLowerCase() === n.toLowerCase())) ordered.push(n);
      });
    });
    if (ordered.length >= 2) { names = ordered; authoritativeNames = true; }
  }
  let ex = null;
  let usedLocalNames = names.length >= 3;

  // Always gather BOTH the local "City, ST" scan AND the AI's body extraction,
  // then UNION them — so the count doesn't swing (2 vs 8) with how much of the
  // slideshow body the reader happened to fetch.
  {
    const exPrompt =
...
ARTICLE: """${String(text).slice(0, 12000)}"""`;
  ...
  const declaredCount = String(text).match(/ARTICLE TITLE:\s*(\d{1,2})\b/i);
  if (declaredCount) {
    const limit = Number(declaredCount[1]);
    if (limit >= 2 && limit <= 20 && names.length > limit) names = names.slice(0, limit);
  }
...
TEXT: """${String(text).slice(0, 8000)}"""`;
    try {
      ...
      if (cleaned.length >= 2) {
        const merged = names.slice();
        cleaned.forEach(n => { if (!merged.some(m => m.toLowerCase() === n.toLowerCase())) merged.push(n); });
        names = merged;                       // union → fuller, more consistent
        if (ex2 && ex2.topic && !(ex && ex.topic)) ex = ex2;
        if (ex2 && ex2.source && !(ex && ex.source)) ex = Object.assign({}, ex, { source: ex2.source });
      }
    } catch (e) {}
...
${noExcerpt ? `No source excerpt is available for ${name} — rely on verified general knowledge only.` : `SOURCE EXCERPT about ${name}: """${excerpt}"""`}`;
    ...
    if (data && Array.isArray(data.sections) && data.sections.length) {
      ...
      data.source = source || (data.source && !/^ai\s*radar/i.test(data.source) ? data.source : "");
      data._topic = topic;     // used to theme footage queries (housing, etc.)
...
function loadStudioMusic(file) {
  ...
  vstudio._userMusicBuffer = null; vstudio._musicBuffer = null;
  // decode to a raw buffer so the uploaded track is captured reliably in exports
  (async () => {
    try {
      if (!vstudio._playCtx) vstudio._playCtx = new (window.AudioContext || window.webkitAudioContext)();
      const arr = await file.arrayBuffer();
      vstudio._userMusicBuffer = await vstudio._playCtx.decodeAudioData(arr.slice(0));
      vstudio._musicBuffer = vstudio._userMusicBuffer;
    } catch (e) { try { console.warn("[audio] could not decode uploaded music", e); } catch(_){} }
  })();
  refreshTimelineClips();
...
function drawNewsBanner(ctx, W, H, elapsed, dsVal, vsOff, dsDur) {
  ...
  const fitWrap = (str, weight, family, startPx, minPx, maxW, maxBlockH, lineHmul) => {
    ...
    const widest = (lns) => {
      ...
      return lns.reduce((m, l) => Math.max(m, ctx.measureText(l).width), 0);
    };
    let lines = wrap(str, `${weight} ${px}px ${fam}`, maxW, 99);
    // 1) shrink (respecting the floor) until the block fits the height
    while (px > minPx && lines.length * px * lineHmul > maxBlockH) {
      px -= Math.max(1, Math.round(px * 0.06));
      lines = wrap(str, `${weight} ${px}px ${fam}`, maxW, 99);
    }
    // 2) GUARANTEE width fit — keep shrinking past the floor if any line is still
    //    wider than the box (long words / smarter-model long headlines), so text
    //    can never spill past the frame edges. Smaller-but-whole beats clipped.
    let guard = 0;
    while (px > 9 && widest(lines) > maxW && guard++ < 200) {
      px -= 1;
      lines = wrap(str, `${weight} ${px}px ${fam}`, maxW, 99);
    }
    return { lines, px };
  ...
  // ══════════════════════════════════════════════════════════
    ...
    if (source) {
      ...
      ctx.font = `600 ${Math.round(U*0.036)}px Inter, sans-serif`; ctx.textAlign = align;
      ctx.fillText(source, cx, lineY+U*0.065);
    }
    ...
    const sc=ctx.createLinearGradient(0,by-U*0.08,0,H);
    sc.addColorStop(0,"rgba(0,0,0,0)"); sc.addColorStop(0.3,"rgba(0,0,0,0.8)"); sc.addColorStop(1,"rgba(0,0,0,0.9)");
    ctx.fillStyle=sc; ctx.fillRect(0,by-U*0.08,W,H-by+U*0.08);
    ctx.fillStyle=ac.bar; ctx.fillRect(W*0.42,by-U*0.025,W*0.16*e,U*0.006);
    ctx.fillStyle="#fff"; ctx.textAlign="center"; ctx.shadowColor="rgba(0,0,0,0.55)"; ctx.shadowBlur=U*0.022;
    ctx.font=`800 ${capPx}px ${vsGetFont("Inter, sans-serif")}`;
    let yy=by+lH*0.75;
    ...
    ctx.fillStyle="rgba(12,10,8,0.94)"; roundRectPath(ctx,aX,cardY,aW,cardH,U*0.028); ctx.fill();
    ctx.strokeStyle=vsHexA(ac.bar,0.55); ctx.lineWidth=1.5; roundRectPath(ctx,aX,cardY,aW,cardH,U*0.028); ctx.stroke();
    ctx.fillStyle=ac.bar; ctx.fillRect(aX,cardY,aW,U*0.009);
    let aY=cardY+U*0.075;
    if(kicker){ctx.fillStyle=ac.bar;ctx.textAlign="left";ctx.font=`700 ${Math.round(U*0.032)}px Inter, sans-serif`;ctx.fillText(kicker.toUpperCase(),aX+aW*0.05,aY);aY+=U*0.07;}
    ...
    ctx.shadowBlur=0;
    if(source){ctx.fillStyle="rgba(210,200,180,0.9)";ctx.font=`700 ${Math.round(U*0.032)}px Inter, sans-serif`;ctx.fillText(source,aX+aW*0.05,aY+lines.length*lH+U*0.07);}
...
function drawInfographic(ctx, W, H, elapsed, tpl, dsVal, vsOff) {
  ...
  stats.forEach((s, i) => {
    ...
    if (style === "bars") {
      ...
      ctx.stroke();
      ctx.lineCap = "butt";
      // value inside — fit to THIS column's width (never spill onto neighbours)
      ctx.textBaseline = "middle";
      ctx.fillStyle = ig.value;
      ctx.textAlign = "center";
      const valMaxW = Math.min(rad * 1.7, slotW * 0.9);
      vsFitFont(ctx, s.value, valMaxW, "800", vsGetFont("Inter, sans-serif"),
        Math.round(Math.min(U * 0.03, rad * 0.6)), Math.round(rad * 0.22));
      let dv = String(s.value);
      if (ctx.measureText(dv).width > valMaxW) {     // still too long → ellipsize
        while (dv.length > 1 && ctx.measureText(dv + "…").width > valMaxW) dv = dv.slice(0, -1);
        dv = dv.replace(/[\s.,\-]+$/, "") + "…";
      }
      ctx.fillText(dv, cxx, cyy);
      // label below
      ctx.fillStyle = ig.label;
...
// ── Meaningful animated scene graphics ────────────────────────────────────
// Vector graphics that EXPLAIN the subject — a radar sweep that discovers
// labelled nodes, a real product UI mock-up, a node network, a data chart —
// instead of decorative particles. Each scene picks the one that fits its own
...
// Draw the chosen graphic. `o`: { accent, items[], value, valueLabel, brand, font }
function drawSceneGraphic(ctx, W, H, kind, t, o) {
  const A = o.accent || "#2FE3C9";
  const F = o.font || "Inter, sans-serif";
  const items = (o.items && o.items.length) ? o.items.slice(0, 5) : ["Signal", "Pattern", "Action"];
  ctx.save();
  ctx.textBaseline = "alphabetic";

  if (kind === "radar") {
    const cx = W / 2, cy = H * 0.545, R = Math.min(W, H) * 0.30;
    for (let i = 1; i <= 4; i++) {
      ctx.beginPath(); ctx.arc(cx, cy, R * i / 4, 0, 7);
      ctx.strokeStyle = vsHexA(A, 0.10 + i * 0.03); ctx.lineWidth = Math.max(1, W * 0.0032); ctx.stroke();
    }
    ctx.strokeStyle = vsHexA(A, 0.10); ctx.beginPath();
    ...
    try {
      ...
      cg.addColorStop(0, vsHexA(A, 0.40)); cg.addColorStop(0.13, vsHexA(A, 0.06));
      cg.addColorStop(0.30, vsHexA(A, 0)); cg.addColorStop(1, vsHexA(A, 0));
      ctx.beginPath(); ctx.moveTo(cx, cy); ctx.arc(cx, cy, R, 0, 7); ctx.closePath();
      ctx.fillStyle = cg; ctx.fill();
    } catch (e) {}
    ctx.beginPath(); ctx.moveTo(cx, cy); ctx.lineTo(cx + Math.cos(a) * R, cy + Math.sin(a) * R);
    ctx.strokeStyle = A; ctx.lineWidth = Math.max(1.5, W * 0.0045);
    ctx.shadowColor = A; ctx.shadowBlur = W * 0.03; ctx.stroke(); ctx.shadowBlur = 0;
    const glyphs = ["▤", "◎", "✳", "◈", "⬡"];
    items.forEach((label, i) => {
      const ang = (-Math.PI / 2) + i * (Math.PI * 2 / items.length);
      const nx = cx + Math.cos(ang) * R * 0.80, ny = cy + Math.sin(ang) * R * 0.80;
      const d = ((a - ang) % (Math.PI * 2) + Math.PI * 2) % (Math.PI * 2);
      const ping = Math.max(0, 1 - d / 0.7);
      ctx.beginPath(); ctx.moveTo(cx, cy); ctx.lineTo(nx, ny);
      ctx.strokeStyle = vsHexA(A, 0.14 + ping * 0.5); ctx.lineWidth = Math.max(1, W * 0.0028); ctx.stroke();
      const r = W * 0.052 * (1 + ping * 0.14);
      ctx.beginPath(); ctx.arc(nx, ny, r, 0, 7);
      ctx.fillStyle = "#0a1420"; ctx.fill();
      ctx.strokeStyle = ping > 0.05 ? A : vsHexA(A, 0.42); ctx.lineWidth = Math.max(1.2, W * 0.0036);
      if (ping > 0.05) { ctx.shadowColor = A; ctx.shadowBlur = W * 0.04 * ping; }
    ...
    ctx.fillStyle = A; ctx.shadowColor = A; ctx.shadowBlur = W * 0.045; ctx.fill(); ctx.shadowBlur = 0;

  } else if (kind === "network") {
    const cx = W / 2, cy = H * 0.545, R = Math.min(W, H) * 0.275;
    const nodes = items.map((l, i) => {
      const ang = (-Math.PI / 2) + i * (Math.PI * 2 / items.length);
      return { x: cx + Math.cos(ang) * R, y: cy + Math.sin(ang) * R, l };
    });
    nodes.forEach((n, i) => {
      const e = Math.min(1, Math.max(0, (t - i * 0.14) / 0.55));
      ctx.beginPath(); ctx.moveTo(cx, cy); ctx.lineTo(cx + (n.x - cx) * e, cy + (n.y - cy) * e);
      ctx.strokeStyle = vsHexA(A, 0.30); ctx.lineWidth = Math.max(1, W * 0.003); ctx.stroke();
      const m = nodes[(i + 1) % nodes.length];
      ctx.beginPath(); ctx.moveTo(n.x, n.y); ctx.lineTo(n.x + (m.x - n.x) * e, n.y + (m.y - n.y) * e);
      ctx.strokeStyle = "rgba(37,99,255,.24)"; ctx.lineWidth = Math.max(1, W * 0.0022); ctx.stroke();
      const p = ((t * 0.55 + i * 0.22) % 1);
      ctx.beginPath(); ctx.arc(cx + (n.x - cx) * p, cy + (n.y - cy) * p, W * 0.009, 0, 7);
      ctx.fillStyle = A; ctx.shadowColor = A; ctx.shadowBlur = W * 0.028; ctx.fill(); ctx.shadowBlur = 0;
    });
    nodes.forEach((n, i) => {
      const e = Math.min(1, Math.max(0, (t - i * 0.14) / 0.55));
      const r = W * 0.055 * e;
      const rg = ctx.createRadialGradient(n.x, n.y - r * 0.3, r * 0.2, n.x, n.y, r);
      rg.addColorStop(0, vsHexA(A, 0.85)); rg.addColorStop(1, "#0a1420");
      ctx.beginPath(); ctx.arc(n.x, n.y, r, 0, 7); ctx.fillStyle = rg; ctx.fill();
    ...
    ctx.beginPath();
    for (let i = 0; i < 6; i++) {
      const ang = i * Math.PI / 3 - Math.PI / 2, hr = W * 0.052;
      i ? ctx.lineTo(cx + Math.cos(ang) * hr, cy + Math.sin(ang) * hr)
        : ctx.moveTo(cx + Math.cos(ang) * hr, cy + Math.sin(ang) * hr);
    }
    ctx.closePath(); ctx.fillStyle = vsHexA(A, 0.16); ctx.fill();
    ctx.strokeStyle = A; ctx.lineWidth = Math.max(1.5, W * 0.0045);
    ...
    ctx.translate(W / 2, y + h / 2); ctx.scale(0.94 + e * 0.06, 0.94 + e * 0.06); ctx.translate(-W / 2, -(y + h / 2));
    ctx.globalAlpha = e;
    roundRectPath(ctx, x, y, w, h, W * 0.032); ctx.fillStyle = "#080d16"; ctx.fill();
    ctx.strokeStyle = vsHexA(A, 0.28); ctx.lineWidth = Math.max(1, W * 0.0032); ctx.stroke();
    const bh = H * 0.030;
    ctx.save(); roundRectPath(ctx, x, y, w, bh, W * 0.032); ctx.clip();
    ctx.fillStyle = "#0d1420"; ctx.fillRect(x, y, w, bh); ctx.restore();
    ["#ff5f57", "#febc2e", "#28c840"].forEach((c, i) => {
      ctx.beginPath(); ctx.arc(x + W * 0.035 + i * W * 0.032, y + bh / 2, W * 0.009, 0, 7);
      ctx.fillStyle = c; ctx.fill();
    });
    roundRectPath(ctx, x + w * 0.30, y + bh * 0.24, w * 0.42, bh * 0.52, bh * 0.26);
    ...
    ctx.fillStyle = "#fff"; ctx.font = `700 ${W * 0.019}px ${F}`; ctx.textAlign = "center";
    ctx.fillText("Launch", x + w * 0.9, ny + W * 0.001);
    const hy = ny + H * 0.050;
    ctx.textAlign = "center";
    const bw2 = w * 0.62;
    roundRectPath(ctx, x + (w - bw2) / 2, hy - W * 0.030, bw2, W * 0.042, W * 0.021);
    ctx.fillStyle = vsHexA(A, 0.12); ctx.fill();
    ctx.strokeStyle = vsHexA(A, 0.42); ctx.lineWidth = 1; ctx.stroke();
    ctx.fillStyle = A; ctx.font = `700 ${W * 0.019}px ${F}`;
    ctx.fillText("✦ " + String(o.badge || "NEW").toUpperCase(), x + w / 2, hy);
    // two-tone headline, measured so the parts never overlap
    ctx.font = `800 ${W * 0.042}px ${F}`;
    const p1 = String(o.uiTitleA || "Built for "), p2 = String(o.uiTitleB || "everyone");
    const w1 = ctx.measureText(p1).width, w2 = ctx.measureText(p2).width;
    const sx0 = x + w / 2 - (w1 + w2) / 2;
    ctx.textAlign = "left";
    ctx.fillStyle = "#fff"; ctx.fillText(p1, sx0, hy + H * 0.046);
    ctx.fillStyle = A; ctx.fillText(p2, sx0 + w1, hy + H * 0.046);
    ctx.textAlign = "center";
    ctx.fillStyle = "rgba(255,255,255,.42)"; ctx.font = `${W * 0.020}px ${F}`;
    ctx.fillText(String(o.uiSub || ""), x + w / 2, hy + H * 0.080);
    const by = hy + H * 0.104;
    roundRectPath(ctx, x + w * 0.22, by, w * 0.26, H * 0.034, H * 0.017); ctx.fillStyle = A; ctx.fill();
    ctx.fillStyle = "#04121a"; ctx.font = `700 ${W * 0.020}px ${F}`;
    ctx.fillText("Start Free", x + w * 0.35, by + H * 0.023);
    roundRectPath(ctx, x + w * 0.53, by, w * 0.25, H * 0.034, H * 0.017);
    ctx.strokeStyle = "rgba(255,255,255,.22)"; ctx.lineWidth = 1.2; ctx.stroke();
    ctx.fillStyle = "rgba(255,255,255,.75)"; ctx.fillText("Book Demo", x + w * 0.655, by + H * 0.023);
    const cy2 = by + H * 0.056, ch = H * 0.135;
    roundRectPath(ctx, x + w * 0.06, cy2, w * 0.88, ch, W * 0.02);
    ctx.fillStyle = "rgba(255,255,255,.045)"; ctx.fill();
    ctx.strokeStyle = "rgba(255,255,255,.10)"; ctx.lineWidth = 1; ctx.stroke();
    ctx.textAlign = "left"; ctx.fillStyle = "rgba(255,255,255,.55)"; ctx.font = `600 ${W * 0.019}px ${F}`;
    ctx.fillText(String(o.cardTitle || "Live metrics"), x + w * 0.10, cy2 + H * 0.024);
    ctx.textAlign = "right"; ctx.fillStyle = A; ctx.font = `700 ${W * 0.017}px ${F}`;
    ctx.fillText("● Active", x + w * 0.90, cy2 + H * 0.024);
    ctx.textAlign = "left";
    (o.metrics || [["Accuracy", "99.4%"], ["Verified", "1.2M+"]]).slice(0, 2).forEach((s, i) => {
      const sx = x + w * 0.10 + i * w * 0.40;
      ctx.fillStyle = "rgba(255,255,255,.40)"; ctx.font = `${W * 0.016}px ${F}`;
      ctx.fillText(s[0], sx, cy2 + H * 0.047);
      ctx.fillStyle = i ? "#ff8a5a" : A; ctx.font = `800 ${W * 0.029}px ${F}`;
      ctx.fillText(s[1], sx, cy2 + H * 0.073);
    });
    const gx = x + w * 0.10, gy = cy2 + H * 0.083, gw = w * 0.80, gh = H * 0.042;
    const pts = [];
    for (let i = 0; i <= 28; i++) {
      const p = i / 28;
      pts.push([gx + gw * p, gy + gh - (gh * (0.28 + 0.30 * Math.sin(p * 4.2) + 0.34 * p))]);
    }
    const prog2 = Math.min(1, Math.max(0, (t - 0.5) / 1.3));
    const n = Math.max(2, Math.floor(pts.length * prog2));
    ctx.beginPath(); ctx.moveTo(pts[0][0], pts[0][1]);
    for (let i = 1; i < n; i++) ctx.lineTo(pts[i][0], pts[i][1]);
    ctx.lineTo(pts[n - 1][0], gy + gh); ctx.lineTo(gx, gy + gh); ctx.closePath();
    const ag = ctx.createLinearGradient(0, gy, 0, gy + gh);
    ag.addColorStop(0, vsHexA(A, 0.34)); ag.addColorStop(1, vsHexA(A, 0));
    ctx.fillStyle = ag; ctx.fill();
    ctx.beginPath(); ctx.moveTo(pts[0][0], pts[0][1]);
    for (let i = 1; i < n; i++) ctx.lineTo(pts[i][0], pts[i][1]);
    ctx.strokeStyle = A; ctx.lineWidth = Math.max(1.5, W * 0.0045);
    ctx.shadowColor = A; ctx.shadowBlur = W * 0.022; ctx.stroke(); ctx.shadowBlur = 0;
    ctx.beginPath(); ctx.arc(pts[n - 1][0], pts[n - 1][1], W * 0.010, 0, 7);
    ctx.fillStyle = "#fff"; ctx.fill();
  }

  // supporting caption under every graphic
  if (o.caption) {
    ctx.globalAlpha = 1;
    ctx.textAlign = "center";
...
function drawSceneMap(ctx, W, H, t, o, A, F, items) {
  ...
  for (let ny = 0; ny <= 1; ny += stepY) {
    for (let nx = 0; nx <= 1; nx += stepX) {
      ...
      const r = W * (0.0040 + depth * 0.0020) * (1 + hot * 0.9);
      ctx.beginPath(); ctx.arc(x, y, r, 0, 7);
      if (hot > 0.05) { ctx.fillStyle = vsHexA(A, 0.55 + hot * 0.45); ctx.shadowColor = A; ctx.shadowBlur = W * 0.012 * hot; }
      else { ctx.fillStyle = vsHexA(A, 0.34 + depth * 0.16); }
...
function drawSceneGraphicExt(ctx, W, H, kind, t, o, A, F, items) {
  ...
  if (kind === "stat") {
    ...
    const cx = W / 2, cy = H * 0.56, R = U * 0.25;
    ctx.strokeStyle = vsHexA(A, 0.28); ctx.lineWidth = W * 0.009;
    ctx.beginPath(); ctx.arc(cx, cy, R, 0, 7); ctx.stroke();
    const sweep = (t * 0.8) % (Math.PI * 2);
    ctx.strokeStyle = A; ctx.lineWidth = W * 0.009; ctx.lineCap = "round"; softGlow(A, W * 0.03);
    ctx.beginPath(); ctx.arc(cx, cy, R, sweep, sweep + Math.PI * 0.55); ctx.stroke(); noGlow();
    its.slice(0, n).forEach((l, i) => {
      ...
      if (e <= 0) return;
      const r = W * 0.064 * e;
      const rg = ctx.createRadialGradient(nx, ny - r * 0.3, r * 0.2, nx, ny, r);
    ...
    const cx = W / 2, cy = H * 0.56, R = U * 0.26;
    const its = items.slice(0, 5);
    ctx.strokeStyle = vsHexA(A, 0.20); ctx.lineWidth = W * 0.004;
    ctx.beginPath(); ctx.arc(cx, cy, R, 0, 7); ctx.stroke();
    its.forEach((l, i) => {
      const ang = t * 0.45 + i * (Math.PI * 2 / its.length);
      const nx = cx + Math.cos(ang) * R, ny = cy + Math.sin(ang) * R;
      const e = ease((t - i * 0.12) / 0.5);
      ctx.strokeStyle = vsHexA(A, 0.28); ctx.lineWidth = W * 0.0025;
      ctx.beginPath(); ctx.moveTo(cx, cy); ctx.lineTo(nx, ny); ctx.stroke();
      const r = W * 0.052 * e;
      const rg = ctx.createRadialGradient(nx, ny - r * 0.3, r * 0.2, nx, ny, r);
      rg.addColorStop(0, vsHexA(A, 0.9)); rg.addColorStop(1, "#0a1420");
      ctx.beginPath(); ctx.arc(nx, ny, r, 0, 7); ctx.fillStyle = rg; ctx.fill();
    ...
    const hg = ctx.createRadialGradient(cx, cy - W * 0.02, W * 0.02, cx, cy, W * 0.07);
    hg.addColorStop(0, vsHexA(A, 0.85)); hg.addColorStop(1, "#0a1420");
    ctx.fillStyle = hg; ctx.fill();
    ctx.strokeStyle = A; ctx.lineWidth = W * 0.005; softGlow(A, W * 0.04); ctx.stroke(); noGlow();
    ctx.fillStyle = "#fff"; ctx.beginPath(); ctx.arc(cx, cy, W * 0.016, 0, 7); ctx.fill();
  ...
  if (introSlide) {
    ...
    if (introSlide._standaloneInfo || introSlide._standaloneNews) {
      ...
      if (introSlide._caption && introSlide._caption.trim()) {
        ...
        ctx.textAlign = "center";
        let capPx = Math.round(U * 0.054);            // +50% larger caption
        ctx.font = `800 ${capPx}px Inter, sans-serif`;
        const cap = introSlide._caption.toUpperCase();
        // shrink if the pill would exceed the frame width
        while (capPx > U * 0.03 && ctx.measureText(cap).width + U * 0.07 > W * 0.92) {
          capPx -= 1; ctx.font = `800 ${capPx}px Inter, sans-serif`;
        }
        const padX = U * 0.038, padY = capPx * 0.46;
...
function drawCard(ctx, W, H, tpl, txt, alpha, subTxt, motion, prog, kind, srcLine) {
  ...
  switch (motion) {
    ...
    case "blur":     blur = (1 - e) * U * 0.045; sc = 1.06 - e * 0.06; break;  // cinematic blur-in
  ...
  let subBottomY = dividerY + U * 0.05;
  if (subTxt) {
    ...
    subBottomY = subStartY + (subLines.length - 1) * subPx * 1.3 + subPx;
    ctx.restore();
  }
  // Large, separate "by Publisher" source line — intro only.
  if (srcLine) {
    const e3 = 1 - Math.pow(1 - Math.max(0, Math.min(1, (prog == null ? 1 : prog) * 1.4 - 0.5)), 3);
    ctx.save();
    ctx.globalAlpha = Math.max(0, alpha) * e3;
    ctx.fillStyle = "#ffffff";
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    let srcPx = Math.round(U * 0.044);                 // much larger than before
    ctx.font = `700 ${srcPx}px Inter, sans-serif`;
    while (srcPx > U * 0.022 && ctx.measureText(srcLine).width > W * 0.86) {
      srcPx -= 1; ctx.font = `700 ${srcPx}px Inter, sans-serif`;
    }
    ctx.fillText(srcLine, cx, subBottomY + U * 0.05 + (1 - e3) * U * 0.03);
    ctx.restore();
  }
  ctx.restore();
}
...
function previewStudioVideo(fromStart) {
  ...
  const loop = () => {
    ...
    if (elapsed >= duration) {
      ...
      if (vstudio.narrationEl) { try { vstudio.narrationEl.currentTime = 0; } catch {} }
    }
...
async function exportStudioVideo() {
  ...
  // (capturing an <audio> element through the graph can come out silent).
  ...
  if (mBuf || nBuf) {
    ...
    } catch (e) { try { console.warn("[audio] export graph failed", e); } catch(_){} }
  }
  ...
  const wantFormat = "mp4";
  let mime = "video/webm";
  let recordedNativeMp4 = false;
  // Native Chrome MP4 recording can report a bogus 30,000 fps timebase and
  ...
  recorder.onerror = (ev) => {
    // Without this, a MediaRecorder failure mid-render leaves the export
    // stuck forever on "Rendering…" with no file and no explanation.
    console.error("MediaRecorder error:", ev && ev.error);
    vsStatus(state.lang === "fa"
    ...
    recorder.onstop = async () => {
      ...
      if (recordedNativeMp4) {
        ...
        if (!webmBlob || !webmBlob.size) {
          ...
          // corrupt/empty file.
          console.error("Export produced no video data (0 recorded chunks).");
          vsStatus(state.lang === "fa"
            ? "ضبط ویدیو ناموفق بود — هیچ فریمی ذخیره نشد. صفحه را رفرش کن و دوباره امتحان کن."
            : "Recording failed — no frames were captured. Refresh the page and try again.");
          vstudio.rendering = false;
          resolve(null);
          return;
        }
        if (format === "mp4") {
          ...
          // If this fails for ANY reason (CDN unreachable, wasm out of
          // memory, browser quirk…) we must NOT just bail out silently —
          // that used to leave the user with a status message claiming a
          ...
          try {
            ...
            ext = "webm";
            vsStatus((state.lang === "fa"
              ? "تبدیل MP4 ناموفق بود — به‌جای آن WebM دانلود می‌شود. خطا: "
              : "MP4 conversion failed — downloading WebM instead. Error: ")
              + (err && err.message ? err.message : String(err)));
            // fall through to the normal save/return-blob logic below with
            // the WebM blob, instead of returning with nothing downloaded.
          }
        } else {
          outBlob = webmBlob;
          ext = "webm";
      ...
      if (!outBlob || !outBlob.size) {
        console.error("Export finished with an empty output blob.");
        vsStatus(state.lang === "fa"
          ? "خروجی ویدیو خالی بود — چیزی دانلود نشد. صفحه را رفرش کن و دوباره امتحان کن."
          : "The exported video was empty — nothing was downloaded. Refresh and try again.");
        vstudio.rendering = false;
        resolve(null);
        return;
      }
  ...
  // start the audio buffer sources exactly as recording begins
  if (vstudio._exportAudio) {
    try {
      const ea = vstudio._exportAudio, actx = ea.actx;
      const t0 = actx.currentTime + 0.08, end = t0 + (ea.dur || 20);
  ...
  // must stay on screen instead of being overwritten by a false "downloaded".
  if (result && !vstudio._returnBlob) vsStatus(state.lang === "fa" ? "ویدیو دانلود شد." : "Video downloaded.");
...
function bindEvents() {
  ...
  const unlockAudio = () => {
    try {
      ...
      if (vstudio._audioCtx.state === "suspended") vstudio._audioCtx.resume().catch(() => {});
    } catch (e) {}
...
/* AI RADAR PRO PROJECT WORKFLOW */
(function aiRadarProProjectWorkflow() {
  const STORAGE_KEY = "airadar.videoStudio.project.v1";
  let saveTimer = 0, importing = false;
  const el = id => document.getElementById(id);
  const cleanSettings = settings => {
    const result = {};
    Object.entries(settings || {}).forEach(([key, value]) => {
      if (!/api|key|token|secret/i.test(key)) result[key] = value;
    });
    return result;
  };
  function projectData() {
    ...
    return {
      format: "airadar-video-project", version: 1, savedAt: new Date().toISOString(),
      name: (el("vsProjectName")?.textContent || "Untitled project").trim(),
      templateId: vstudio.templateId || "", activeSlide: vstudio.activeSlide || 0,
      controls: typeof vsSnapshot === "function" ? cleanSettings(vsSnapshot()) : {},
      slides: vstudio.slides.map((s, index) => ({
        id: s.id || ("scene-" + (index + 1)), duration: Number(s.duration) || 4,
        isVideo: !!s.isVideo, isIntro: !!s.isIntro, isOutro: !!s.isOutro,
        introMain: s.introMain || "", introSub: s.introSub || "", introMotion: s.introMotion || "",
        caption: s._caption || "", standaloneInfo: !!s._standaloneInfo, standaloneNews: !!s._standaloneNews,
        settings: cleanSettings(s.settings),
        mediaUrl: s.url && !String(s.url).startsWith("blob:") ? s.url : "",
        mediaMissing: !!(s.mediaEl && (!s.url || String(s.url).startsWith("blob:")))
      }))
    };
  }
  function setProjectState(text, dirty = false) {
    const node = el("vsProjectState"); if (!node) return;
    node.textContent = text; node.classList.toggle("is-dirty", dirty);
  }
  function updateProjectUI() {
    const count = vstudio.slides.length;
    const duration = Math.round(vstudio.slides.reduce((sum, s) => sum + (Number(s.duration) || 0), 0) * 10) / 10;
    if (el("vsProjectStats")) el("vsProjectStats").textContent = count + " scene" + (count === 1 ? "" : "s") + " · " + duration + "s";
    const i = vstudio.activeSlide, has = count > 0;
    if (el("vsSceneDuplicate")) el("vsSceneDuplicate").disabled = !has;
    if (el("vsSceneUp")) el("vsSceneUp").disabled = !has || i <= 0;
    if (el("vsSceneDown")) el("vsSceneDown").disabled = !has || i >= count - 1;
  }
  function autosave() {
    if (importing) return; clearTimeout(saveTimer); setProjectState("Unsaved changes", true);
    saveTimer = setTimeout(() => {
      try { localStorage.setItem(STORAGE_KEY, JSON.stringify(projectData())); setProjectState("Autosaved"); }
      catch { setProjectState("Autosave unavailable", true); }
    }, 650);
  }
  function loadMediaForScene(scene, url, isVideo) {
    if (!url) return;
    if (isVideo) {
      const media = document.createElement("video"); media.src = url; media.muted = true; media.playsInline = true;
      media.addEventListener("loadedmetadata", () => { scene.mediaEl = media; scene.ready = true; renderSlideList(); drawStudioFrame(vstudio.position || 0); }, { once: true });
    } else {
      const media = new Image();
      media.onload = () => { scene.mediaEl = media; scene.ready = true; renderSlideList(); drawStudioFrame(vstudio.position || 0); };
      media.src = url;
    }
  }
  function applyProject(data) {
    if (!data || data.format !== "airadar-video-project" || !Array.isArray(data.slides)) throw new Error("This is not a valid AI Radar project.");
    importing = true;
    try {
      ...
      data.slides.forEach(raw => {
        const scene = {
          id: raw.id || ("scene-" + crypto.randomUUID()), duration: Math.max(1, Number(raw.duration) || 4),
          isVideo: !!raw.isVideo, isIntro: !!raw.isIntro, isOutro: !!raw.isOutro,
          introMain: raw.introMain || "", introSub: raw.introSub || "", introMotion: raw.introMotion || "rise",
          _caption: raw.caption || "", _standaloneInfo: !!raw.standaloneInfo, _standaloneNews: !!raw.standaloneNews,
          settings: cleanSettings(raw.settings), url: raw.mediaUrl || "", mediaEl: null, ready: false, _mediaMissing: !!raw.mediaMissing
        };
        vstudio.slides.push(scene); loadMediaForScene(scene, scene.url, scene.isVideo);
      });
      vstudio.activeSlide = Math.min(Math.max(0, Number(data.activeSlide) || 0), Math.max(0, vstudio.slides.length - 1));
      if (el("vsProjectName")) el("vsProjectName").textContent = data.name || "Untitled project";
      renderSlideList(); if (vstudio.slides.length) selectSlide(vstudio.activeSlide);
      updateProjectUI(); setProjectState("Project loaded"); localStorage.setItem(STORAGE_KEY, JSON.stringify(projectData()));
      const missing = data.slides.filter(s => s.mediaMissing).length;
      vsStatus(missing ? "Project opened. Reattach media in " + missing + " scene" + (missing === 1 ? "" : "s") + "; local uploads are never embedded for privacy." : "Project opened successfully.");
    } finally { importing = false; }
  }
  function downloadProject() {
    const data = projectData();
    const safeName = (data.name || "airadar-project").replace(/[^a-z0-9_-]+/gi, "-").replace(/^-|-$/g, "") || "airadar-project";
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: "application/json" });
    const link = document.createElement("a"); link.href = URL.createObjectURL(blob); link.download = safeName + ".airadar"; link.click();
    setTimeout(() => URL.revokeObjectURL(link.href), 1000); setProjectState("Project file saved");
  }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (list) new MutationObserver(() => { updateProjectUI(); autosave(); }).observe(list, { childList: true, subtree: true });
    el("vsProjectSave")?.addEventListener("click", downloadProject);
    el("vsSceneDuplicate")?.addEventListener("click", duplicateScene);
    el("vsSceneUp")?.addEventListener("click", () => moveScene(-1));
    el("vsSceneDown")?.addEventListener("click", () => moveScene(1));
    ...
    el("vsProjectOpen")?.addEventListener("change", async event => {
      const file = event.target.files?.[0]; if (!file) return;
      try { applyProject(JSON.parse(await file.text())); } catch (error) { vsStatus(error.message || "Could not open that project."); }
      event.target.value = "";
    });
    ...
    document.addEventListener("keydown", event => {
      ...
      if (command && event.key.toLowerCase() === "s" && active) { event.preventDefault(); downloadProject(); }
      if (command && event.key.toLowerCase() === "d" && active) { event.preventDefault(); duplicateScene(); }
    });
    // Never auto-open a previous/default project after refresh. Saved project
    // data is preserved and can still be opened explicitly by the user.
    updateProjectUI();
