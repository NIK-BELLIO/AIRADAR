...
ARTICLE: """${String(text).slice(0, 12000)}"""`;
  ...
  vsRenderBatchList();
  vsAutoStatus(fa ? `${names.length} مورد پیدا شد — در حال ساخت ویدئوها…`
                  : `Found ${names.length} items — building videos…`);

  // 2) SCRIPT each item from the SAME article (facts pulled at write time,
  //    so each video is accurate to what the article says about that item).
...
From the ARTICLE below, write a complete, professional short-form video script ENTIRELY about "${name}" (part of "${topic}"). Use ONLY what the ARTICLE actually says about "${name}" — its real figures, prices, ranks and reasons. Ignore the other items entirely. Never invent facts.
Return ONLY valid compact JSON (no markdown, no commentary):
...
RULES:
1. EXACTLY ${perRange} content sections (besides intro/outro), built only from what the article says about ${name}.
2. INFOGRAPHIC only where the article gives real numbers for ${name} (prices, %, ranks). Otherwise TEXT.
3. Never two infographics in a row. Vary text styles for rhythm.
4. title MUST be exactly "${name}". Every line concrete and specific to ${name}.
ARTICLE: """${String(text).slice(0, 7000)}"""`;
    let data = null;
