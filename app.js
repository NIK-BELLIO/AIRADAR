// Never ship provider secrets in this public static app. Users can optionally
// provide a publishable Pollinations key in the Studio; it is kept in memory
...
const i18n = {
  en: {
    ...
    vVoiceoverLbl: "\ud83c\udf99\ufe0f AI voiceover narration (free)",
  ...
  fa: {
    ...
    vVoiceoverLbl: "\ud83c\udf99\ufe0f روایت صوتی هوش مصنوعی (رایگان)",
...
// AI chat for the auto-builder. A user-supplied publishable Pollinations key
// unlocks the stronger models; keyless legacy endpoints remain a best-effort
// fallback so the basic editor still works without an account.
async function vsAutoAiChat(prompt, opts) {
  ...
  async function postChat(endpoint, model, useJson, viaProxy) {
    ...
    const pollenKey = vsPollinationsKey();
    if (pollenKey) headers["Authorization"] = "Bearer " + pollenKey;
...
{"title":"the core story in max 6 words","subtitle":"max 8 words that add real context","kicker":"1-2 ALL-CAPS category words","source":"the real publication name or empty string","language":"ISO language code","angle":"one sentence explaining the editorial angle","music":{"mood":"tense|hopeful|investigative|urgent|inspiring|neutral","energy":"low|medium|high","bpm":92},"intro":{"main":"a sharp 3-6 word hook headline","sub":"max 8 words framing the story","narration":"a natural 1-2 sentence spoken hook"},"sections":[{"type":"infographic","caption":"2-3 words","title":"chart headline (max 5 words)","narration":"2-3 spoken sentences that explain why the verified figures matter","evidence":"short source-grounded fact supporting this scene","stats":[{"label":"short label","value":"formatted e.g. $2.4B or 34%","num":2400000000}],"chartType":"bars|donut|pills|comparison|ranking"},{"type":"text","caption":"2-3 words","headline":"one vivid, specific sentence (max 12 words)","narration":"2-3 broadcast-quality spoken sentences with context and consequence","evidence":"short source-grounded fact supporting this scene","style":"title-center|title-left|bold-statement|quote|caption|annotation|badge|magazine-cover"}],"outro":{"main":"3-5 word takeaway","sub":"max 6 words","narration":"one memorable closing sentence"}}
...
9. Write narration for the EAR, not the screen: natural transitions, varied sentence length, no bullet-list rhythm, no hype clichés.
10. Separate fact from inference. Do not invent numbers, quotes, dates, rankings or attribution. Put only source-supported claims in evidence.
11. Keep the SOURCE language unless the user clearly asks in another language. Narration and on-screen copy must use the same language.
12. Build a narrative arc: hook, essential context, strongest evidence, consequence/why it matters, then takeaway. Each scene must advance the story.
...
function vsAssembleFromSections(data, skipFootage) {
  ...
  // Estimate readable scene time from its dedicated spoken copy. This keeps
  // the visuals paced to the narration instead of cutting every scene at six
  // seconds regardless of what the voice is saying.
...
async function vsGenerateAiMusic(data) {
  ...
  const prompt = `${mood} ${energy}-energy cinematic news underscore, ${bpm} BPM, instrumental only, no vocals, clear opening, subtle build, restrained ending, designed under spoken narration`;
  try {
    const ctrl = new AbortController();
    const tm = setTimeout(() => ctrl.abort(), 90000);
    let resp;
    try {
      resp = await fetch("https://gen.pollinations.ai/v1/audio/speech", {
        method: "POST", signal: ctrl.signal,
        headers: { "Content-Type": "application/json", "Authorization": "Bearer " + key },
        body: JSON.stringify({ model: "elevenmusic", input: prompt, duration, instrumental: true, response_format: "mp3" })
      });
    } finally { clearTimeout(tm); }
...
async function vsGenerateNarration(data, voice) {
  ...
  if (key) {
    try {
      ...
      try {
        resp = await fetch("https://gen.pollinations.ai/v1/audio/speech", {
          method: "POST", signal: ctrl.signal,
          headers: { "Content-Type": "application/json", "Authorization": "Bearer " + key },
          body: JSON.stringify({ model: "tts-1", input: script, voice: v, response_format: "mp3" })
        });
      } finally { clearTimeout(tm); }
      const type = (resp.headers.get("content-type") || "").toLowerCase();
    ...
    } catch (e) { /* use the keyless fallback below */ }
  }

  if (!blob && script.length <= 500) {
    ...
    try {
      const url = "https://api.streamelements.com/kappa/v2/speech?voice=" +
        encodeURIComponent(seMap[v] || "Joanna") + "&text=" + encodeURIComponent(script);
  ...
  if (!blob) {
    vsStatus(state.lang === "fa"
      ? "ساخت گویندگی ناموفق بود. یک کلید انتشارپذیر Pollinations با پیشوند pk_ وارد کن."
      : "Voice generation failed. Add a Pollinations publishable key (pk_) in Studio settings.");
    return null;
  }
...
// Kept temporarily for comparison while the new endpoint rolls out.
async function vsGenerateNarrationLegacy(data, voice) {
  try {
    const parts = [];
    if (data.intro) parts.push(String(data.intro.narration || data.intro.main || "").replace(/[#*_`]/g, ""));
    (data.sections || []).forEach(sec => {
      const line = sec.narration || sec.headline || sec.title || "";
      if (line) parts.push(String(line).replace(/[#*_`]/g, ""));
    });
    if (data.outro) parts.push(String(data.outro.narration || data.outro.main || "").replace(/[#*_`]/g, ""));
...
async function vsAutoGenerateBackgrounds(data) {
  ...
  const tryPexels = async (q, i) => {
    let m = await vsFetchPexelsClip(q, pexelsKey, aspect, i);
    if (!m) m = await vsFetchPexelsPhoto(q, pexelsKey, aspect, i);
    return m;
  };
  ...
  const genOne = async (s, i) => {
    ...
    if (isTitle) {
      ...
      query = (houseViews[i % houseViews.length] + (firstPlace ? " " + firstPlace : ""));
...
ARTICLE: """${String(text).slice(0, 12000)}"""`;
    ...
    if (aiNames.length && aiNames.length <= 22) {
...
4. Include AT LEAST ONE "infographic" section with EXACTLY 3 stats, each having a real/realistic numeric value (e.g. "$1,250", "18%", "#4"). Never two infographics in a row. Keep stat labels ≤14 chars and values ≤9 chars. An infographic with no numbers is forbidden — make it a "text" section instead.
6. Write like a senior analyst: every headline is specific and professional (a concrete claim with a figure or comparison), NEVER vague or generic. Forbidden vague lines like "Reset in 10 days" or "Strong market" — always say WHAT and BY HOW MUCH.
5. "source" = the real outlet/publication name only (no "by", no URL); leave "" if unknown.
${cityFound ? 'SOURCE TEXT about ' + name + ': """' + excerpt + '"""' : 'Write from your knowledge of ' + name + ', strictly on the topic "' + topic + '".'}`;
...
(function aiRadarProProjectWorkflow() {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    updateProjectUI();
    try { const saved = JSON.parse(localStorage.getItem(STORAGE_KEY) || "null"); if (saved && !vstudio.slides.length) applyProject(saved); } catch {}
  });
})();
