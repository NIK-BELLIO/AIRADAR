...
function vsAssembleFromSections(data, skipFootage) {
  ...
  sections.forEach((sec, i) => {
    ...
    if (sec.type === "infographic" && Array.isArray(sec.stats) && sec.stats.length) {
      ...
      // Text / narrative slide — vary the style for contrast between scenes
      const validStyles = ["title-center", "bold-statement", "quote", "title-left", "caption"];
      let style = validStyles.includes(sec.style) ? sec.style : "title-center";
      if (style === vstudio._lastNewsStyle) {           // avoid two identical in a row
        const alt = validStyles.filter(s => s !== style);
        style = alt[i % alt.length];
      }
