...
async function vsEnsureDefaultMusic() {
  ...
  try {
    ...
    lp.connect(master);
    // a calm Cmaj9-ish pad — gentle, non-distracting
    const notes = [130.81, 196.00, 261.63, 329.63, 392.00];
    notes.forEach((f, i) => {
      [0, 1.5].forEach(det => {                       // slight detune layer for warmth
        const o = ctx.createOscillator();
        o.type = i % 2 ? "sine" : "triangle";
        o.frequency.value = f; o.detune.value = det;
        const g = ctx.createGain(); g.gain.value = 0.0;
        // slow swell in/out per voice, staggered
        const t0 = i * 0.6, peak = 0.18 / (1 + i * 0.15);
        g.gain.setValueAtTime(0.0001, 0);
        g.gain.linearRampToValueAtTime(peak, t0 + 3);
        g.gain.setValueAtTime(peak, dur - 4);
        g.gain.linearRampToValueAtTime(0.0001, dur);
        // gentle tremolo
        const lfo = ctx.createOscillator(); lfo.frequency.value = 0.08 + i * 0.02;
        const lg = ctx.createGain(); lg.gain.value = peak * 0.4;
        lfo.connect(lg); lg.connect(g.gain); lfo.start(0); lfo.stop(dur);
        o.connect(g); g.connect(lp); o.start(0); o.stop(dur);
      });
    });
...
async function vsGenerateNarration(data, voice) {
  try {
    ...
    if (script.length > 900) script = script.slice(0, 900);   // keep it short

    const v = voice || ((document.querySelector("#vsVoice") || {}).value) || "nova";
    let url = "https://text.pollinations.ai/" + encodeURIComponent(script) +
      "?model=openai-audio&voice=" + encodeURIComponent(v);
    // Pass the key as a query token, NOT an Authorization header — a custom
    // header forces a CORS preflight that the audio endpoint rejects (which is
    // why the voiceover came out silent in the export).
    if (VS_POLLINATIONS_KEY) url += "&token=" + encodeURIComponent(VS_POLLINATIONS_KEY);
    const ctrl = new AbortController();
    ...
    if (!blob || blob.size < 1200) throw new Error("tts empty");
