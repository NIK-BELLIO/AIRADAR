...
function vsAssembleFromSections(data, skipFootage) {
  ...
  sections.forEach((sec, i) => {
    ...
    if (sec.type === "infographic" && Array.isArray(sec.stats) && sec.stats.length) {
      // Clamp every label/value so text never overflows the chart chrome.
      const clampLabel = (s) => {
      ...
      sec.stats = sec.stats.slice(0, 5).map(s => ({
        label: clampLabel(s.label),
        value: String(s.value != null ? s.value : "").replace(/\s+/g, " ").trim().slice(0, 14),
        num: typeof s.num === "number" ? s.num : (parseFloat(String(s.value).replace(/[^0-9.\-]/g, "")) || 0)
      }));
