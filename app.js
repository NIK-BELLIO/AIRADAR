...
const i18n = {
  en: {
    ...
    vAutoBatchLbl: "🎬 Make a separate video for each item/city in the link",
    vUsePexelsLbl: "🎥 Use real stock footage (Pexels) — needs a free API key",
  ...
  fa: {
    ...
    vAutoBatchLbl: "🎬 برای هر شهر/مورد داخل لینک یک ویدئوی جداگانه بساز",
    vUsePexelsLbl: "🎥 استفاده از فوتیج واقعی (Pexels) — به یک کلید رایگان API نیاز دارد",
...
// Fetch a real stock video clip from Pexels (needs a free API key). Returns a
// ready, CORS-clean <video> element, or null on any failure (then we fall
// back to a generated image so the build never breaks).
async function vsFetchPexelsClip(query, key, aspect) {
  ...
  for (const v of vids) {
    ...
    if (files[0]) { bestFile = files[0]; break; }
  }
  if (!bestFile) return null;
  return await new Promise((resolve) => {
    const vid = document.createElement("video");
    vid.crossOrigin = "anonymous";          // needed so the canvas can record it
    ...
    vid.onloadeddata = () => {
      ...
      resolve(vid.videoWidth ? vid : null);
    };
    vid.onerror = () => { if (!done) { done = true; clearTimeout(timer); resolve(null); } };
    vid.src = bestFile.link;
  });
}
...
async function vsAutoGenerateBackgrounds(data) {
  ...
  for (let i = 0; i < slides.length; i++) {
    ...
    // ── REAL footage first (Pexels), if enabled and a key is provided ──
    const pexelsOn = document.querySelector("#vsUsePexels") && document.querySelector("#vsUsePexels").checked;
    const pexelsKey = (document.querySelector("#vsPexelsKey") || {}).value;
    if (pexelsOn && pexelsKey && pexelsKey.trim()) {
      vsAutoStatus(state.lang === "fa" ? `در حال یافتن فوتیج واقعی صحنه ${i + 1}…`
                                       : `Finding real footage for scene ${i + 1}…`);
      try {
        const query = (firstPlace ? firstPlace + " " : "") + (s._caption || subject);
        const clip = await vsFetchPexelsClip(query.trim(), pexelsKey.trim(), vsVal("#vsAspect", "9:16"));
        if (clip) {
          s.mediaEl = clip; s.isVideo = true; s.ready = true; s.url = clip.currentSrc || clip.src;
...
async function vsBuildBatchFromArticle(text, tone, lenChoice) {
  ...
  // 1) EXTRACT every item the article profiles + its real facts (article-only)
  const exPrompt =
`You are a meticulous research analyst. Read the ARTICLE and extract EVERY distinct item it profiles (e.g. cities, products, companies, people) in the order they appear.
For EACH item collect ONLY concrete facts the article actually states about it — figures, prices, ranks, reasons, descriptions. Never invent facts that aren't in the text.
Return ONLY compact JSON (no markdown, no commentary):
{"topic":"overall subject in <=6 words","source":"the outlet/publication name if identifiable else a fitting newsroom label","items":[{"name":"item name exactly as written in the article","location":"city, state/country if applicable else empty","facts":["specific fact pulled from the article", "..."]}]}
Each item needs 3 to 8 specific facts. Include ALL items the article profiles.
ARTICLE: """${String(text).slice(0, 12000)}"""`;

  let ex;
  try { ex = vsParseAiJson(await vsAutoAiChat(exPrompt)); }
  catch (e) { ex = null; }

  const items = ex && Array.isArray(ex.items) ? ex.items.filter(it => it && it.name) : [];
  if (items.length < 2) {
  ...
  const source = ex.source || "AI Radar — Newsroom";
  const perRange = lenChoice === "short" ? "3" : lenChoice === "long" ? "5" : "4";
  const guide = vsToneGuide(tone);

  vstudio.batchVideos = [];
  vstudio.batchCurrent = -1;
  vsRenderBatchList();   // shows the skeleton list immediately

  // 2) SCRIPT each item into its own tight, accurate video
  for (let i = 0; i < items.length; i++) {
    const it = items[i];
    const facts = Array.isArray(it.facts) ? it.facts.join("\n• ") : String(it.facts || "");
    vsAutoStatus(fa ? `ساخت فیلمنامهٔ ${i + 1} از ${items.length}: ${it.name}…`
                    : `Scripting ${i + 1} of ${items.length}: ${it.name}…`);
    const sPrompt =
`You are an award-winning ${guide}
Write a complete, professional short-form video script ENTIRELY about "${it.name}"${it.location ? " (" + it.location + ")" : ""}, part of the topic "${topic}".
Use ONLY the FACTS below — every line must be accurate to them. Do NOT invent numbers or claims not present.
FACTS:
• ${facts}

Return ONLY valid compact JSON (no markdown):
{"title":"${it.name}","subtitle":"max 8 words of real context","kicker":"1-2 ALL-CAPS words","source":"${source}","palette":"fire|ocean|forest|gold|neon|mono","intro":{"main":"3-6 word hook about ${it.name}","sub":"max 8 words"},"sections":[{"type":"infographic","caption":"2-3 words","title":"chart headline","stats":[{"label":"short label","value":"e.g. $1,250 or 18%","num":1250}],"chartType":"bars|donut|pills|comparison|ranking"},{"type":"text","caption":"2-3 words","headline":"one vivid specific sentence (max 12 words)","style":"title-center|bold-statement|quote|title-left|magazine-cover|neon-title"}],"outro":{"main":"3-5 word takeaway","sub":"max 6 words"}}

RULES:
1. EXACTLY ${perRange} content sections (besides intro/outro), built only from the FACTS.
2. INFOGRAPHIC only where the FACTS contain real numbers (prices, %, ranks). Otherwise use TEXT.
3. Never two infographics in a row. Vary text styles.
4. title MUST be "${it.name}". Keep every line concrete and specific to this item.`;
    let data = null;
    try { data = vsParseAiJson(await vsAutoAiChat(sPrompt)); } catch (e) { data = null; }
    if (data && Array.isArray(data.sections) && data.sections.length) {
      if (!data.title) data.title = it.name;
      data._batchName = it.name;
      data._location = it.location || "";
      vstudio.batchVideos.push({ name: it.name, location: it.location || "", data });
      vsRenderBatchList();
    }
  }
  ...
  vsAutoStatus(fa ? `${vstudio.batchVideos.length} ویدئو ساخته شد — اولی باز شد. روی هر شهر بزن تا بازش کنی.`
                  : `Built ${vstudio.batchVideos.length} videos — opened the first. Click any item to load it.`);
...
async function vsLoadBatchVideo(i) {
  ...
  vsRenderBatchList();
  // generate footage for THIS video on demand (lazy — keeps it responsive)
  if (document.querySelector("#vsAutoImages") && document.querySelector("#vsAutoImages").checked) {
    vsAutoStatus(fa ? `در حال ساخت فوتیج برای ${v.name}…` : `Generating footage for ${v.name}…`);
...
function vsRenderBatchList() {
  ...
  vids.forEach((v, i) => {
    ...
    html += `<button type="button" class="vs-batch-item${active}" data-batch="${i}">
      <span class="vs-batch-name">${esc(v.name)}</span>
      <span class="vs-batch-meta">${v.location ? esc(v.location) + " · " : ""}${n ? (fa ? n + " صحنه" : n + " scenes") : ""}</span>
    </button>`;
  });
  box.innerHTML = html;
  box.querySelectorAll(".vs-batch-item").forEach(btn => {
...
function bindEvents() {
  ...
  on("#vsUsePexels", "change", () => {
    ...
    if (w) w.style.display = ($("#vsUsePexels") && $("#vsUsePexels").checked) ? "block" : "none";
  });
