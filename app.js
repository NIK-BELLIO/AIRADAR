...
async function vsAutoAiChat(prompt, opts) {
  ...
  const userKey = ((document.querySelector("#vsApiKey") || {}).value || "").trim();
...
async function vsEnsureDefaultMusic() {
  if (vstudio._defaultMusicEl) return vstudio._defaultMusicEl;
  try {
    ...
    vstudio._defaultMusicEl = el; vstudio._defaultMusicUrl = url;
    return el;
  } catch (e) { return null; }
}
...
(function bindDashboard() {
  function wire() {
    ...
    if (openBtn) openBtn.addEventListener("click", () => { window.location.href = "monitor.html"; });
