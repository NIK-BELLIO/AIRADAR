...
async function vsBuildBatchFromArticle(text, tone, lenChoice) {
  ...
  // 1) EXTRACT the list of items. For city listicles a LOCAL "City, ST" scan
  //    is far MORE reliable than the AI, which tends to grab sub-sections
  //    (budget categories like "Home", "Groceries", "Subscriptions"). So try
  //    local FIRST; only ask the AI when the article isn't a city list.
...
ARTICLE: """${String(text).slice(0, 12000)}"""`;
    ...
    for (let attempt = 0; attempt < 2 && aiNames.length < 2; attempt++) {
      if (vstudio._batchCancel) return false;
      ...
      aiNames = resolveItems(ex)
        .map(it => typeof it === "string" ? it : (it && (it.name || it.title)) || "")
        .map(s => String(s).trim()).filter(Boolean);
      aiNames = aiNames.filter((n, idx) => aiNames.indexOf(n) === idx);
    }
    // If the AI massively over-extracted but we have SOME local cities, trust local.
