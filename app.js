...
// Recolor a logo (image OR current video frame) to a single brand colour by
// keeping its alpha shape and flooding the opaque pixels with `color`. Ideal
// for monochrome wordmarks / motion logos that should match the template.
function vsTintedLogo(srcEl, color, w, h) {
  if (!color) return srcEl;
  const cw = Math.max(2, Math.round(w)), ch = Math.max(2, Math.round(h));
  const oc = vstudio._logoTintCanvas || (vstudio._logoTintCanvas = document.createElement("canvas"));
  if (oc.width !== cw || oc.height !== ch) { oc.width = cw; oc.height = ch; }
  const c = oc.getContext("2d");
  c.globalCompositeOperation = "source-over";
  c.clearRect(0, 0, cw, ch);
  try { c.drawImage(srcEl, 0, 0, cw, ch); } catch (e) { return srcEl; }
  c.globalCompositeOperation = "source-in";   // paint only where the logo is opaque
  c.fillStyle = color;
...
function vsDrawLogo(ctx, W, H, elapsed, dsLocal, tpl) {
  ...
  // ── LOGO OVERLAY — professional motion + placement + frame styles ──
  {
    ...
    // brand-colour tint (original / match template / fixed colour). Applies to
    // PNG and animated (video) logos alike — recolours the mark per frame.
    const tintColor = vsLogoTintColor(tpl);
    const drawEl = tintColor ? vsTintedLogo(logo, tintColor, lw, lh) : logo;
    ...
    if (motion === "letters" && vstudio.logoSegs && vstudio.logoSrcCanvas) {
      ...
      // tint the whole source canvas once if a brand colour is active
      let src = vstudio.logoSrcCanvas;
      if (tintColor) src = vsTintedLogo(src, tintColor, src.width, src.height);
...
const VS_CONTROLS = [
  ...
  "#vsMotion", "#vsTextAnim", "#vsOverlay",
  "#vsInfoOn", "#vsInfoJson", "#vsInfoStyle", "#vsInfoPos", "#vsInfoMotion",
  "#vsLogoPos", "#vsLogoMotion", "#vsLogoStyle", "#vsLogoColor", "#vsLogoTint", "#vsLogoSize", "#vsLogoStart", "#vsLogoDur", "#vsLogoShow", "#vsIntro", "#vsIntroSub", "#vsIntroMotion", "#vsOutro",
...
function bindEvents() {
  ...
  const vsLiveControls = [
    ...
    "#vsIntro", "#vsIntroSub", "#vsIntroMotion", "#vsOutro",
    "#vsLogoPos", "#vsLogoMotion", "#vsLogoStyle", "#vsLogoColor", "#vsLogoTint", "#vsLogoSize", "#vsLogoStart", "#vsLogoDur", "#vsLogoShow"
  ];
  const vsRefresh = () => {
    refreshTimelineClips();
