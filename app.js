...
async function vsEnsureDefaultMusic() {
  try {
    ...
    const bassLP = ctx.createBiquadFilter(); bassLP.type = "lowpass"; bassLP.frequency.value = 260; bassLP.connect(comp);
    ...
    const padLP = ctx.createBiquadFilter(); padLP.type = "lowpass"; padLP.frequency.value = 1600; padLP.Q.value = 0.7; padLP.connect(comp);
    ...
    shim.connect(sg); sg.connect(comp); shim.start(0); shim.stop(dur);
...
async function vsLoadBatchVideo(i) {
  ...
  if (musicTog && musicTog.checked && !vstudio._userMusic) {
    ...
    if (dm) { vstudio.musicEl = dm; try { console.log("[audio] music ready for", v.name); } catch(e){} }
    else { try { console.warn("[audio] music generation FAILED"); } catch(e){} }
  } else if (musicTog && !musicTog.checked && !vstudio._userMusic) {
    vstudio.musicEl = null;
  }
  ...
  if (voTog && voTog.checked) {
    if (!v._narrationEl && !vstudio._batchCancel) {
      ...
      if (nar) { v._narrationEl = nar.el; v._narrationUrl = nar.url; try { console.log("[audio] voiceover ready for", v.name); } catch(e){} }
...
async function exportStudioVideo() {
  ...
  try {
    const musicTog = document.querySelector("#vsMusic");
    if (musicTog && musicTog.checked && !vstudio._userMusic && !vstudio.musicEl) {
      const dm = await vsEnsureDefaultMusic(); if (dm) vstudio.musicEl = dm;
    } else if (musicTog && !musicTog.checked && !vstudio._userMusic) {
      vstudio.musicEl = null;
    }
    const voTog = document.querySelector("#vsVoiceover");
    ...
    if (voTog && voTog.checked) {
      if (cur && !cur._narrationEl) {
        ...
        if (nar) { cur._narrationEl = nar.el; cur._narrationUrl = nar.url; }
      }
  ...
  // mix in music + AI narration if present (shared record destination)
  let tracks = [...canvasStream.getVideoTracks()];
  if (vstudio.musicEl || vstudio.narrationEl) {
    try {
      // A MediaElementSource can only be created ONCE per audio element. We
      // build the graph lazily and reuse it; both music and narration feed the
      // same recording destination so the export captures the mix.
      ...
      const wire = (el, gain) => {
        ...
        try {
          const src = vstudio._audioCtx.createMediaElementSource(el);
          if (gain != null) {
            const g = vstudio._audioCtx.createGain(); g.gain.value = gain;
            src.connect(g); g.connect(vstudio._musicDest); g.connect(vstudio._audioCtx.destination);
          } else {
            src.connect(vstudio._musicDest); src.connect(vstudio._audioCtx.destination);
          }
          el._wired = true;
        } catch (e) {}
      };
      wire(vstudio.musicEl, vstudio.narrationEl ? 0.35 : null);   // duck music under voice
      wire(vstudio.narrationEl, null);
  ...
  if (vstudio.narrationEl) {
    try {
      vstudio.narrationEl.playbackRate = parseFloat((document.querySelector("#vsVoiceSpeed") || {}).value) || 1;
      vstudio.narrationEl.currentTime = 0; await vstudio.narrationEl.play().catch(() => {});
    } catch {}
  }
