  ...
  if (vstudio.logoEl) {
    ...
    if (vstudio.logoIsVideo) {
      ...
      if (vd > 0) {
        let want;
        if (lDurIn > 0) {
          // SMART: fit the whole logo animation into the chosen duration.
          // duration 3s → play the full clip across 3s; 9s → across 9s.
          const prog = Math.min(1, localT / lDurIn);   // 0..1 across the window
          want = prog * Math.max(0, vd - 0.05);
        } else {
          // no fixed duration → just loop at native speed
          want = localT % vd;
        }
        if (Math.abs((logo.currentTime || 0) - want) > 0.06) {
          try { logo.currentTime = want; } catch (e) {}
        }
      }
      // we drive currentTime ourselves; keep the element paused to avoid drift
...
(function bindDashboard() {
  ...
  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", wire);
      ...
      /* logo auto-brand button + tip */
      .vs-logo-layer { position: relative; height: 26px; margin-bottom: 6px; }
      .vs-logo-layer-bar { position: absolute; top: 0; height: 22px; display: flex; align-items: center; gap: 6px; padding: 0 14px; border-radius: 7px; background: linear-gradient(90deg, rgba(0,212,255,0.3), rgba(216,183,106,0.3)); border: 1px solid rgba(0,212,255,0.6); font-size: 11px; font-weight: 700; color: #fff; white-space: nowrap; box-shadow: 0 2px 8px rgba(0,0,0,0.3); cursor: grab; user-select: none; overflow: hidden; }
      .vs-logo-layer-bar:active { cursor: grabbing; }
      .vs-logo-handle { position: absolute; top: 0; bottom: 0; width: 11px; cursor: ew-resize; z-index: 2; }
      .vs-logo-handle::after { content: ""; position: absolute; top: 4px; bottom: 4px; width: 3px; border-radius: 2px; background: rgba(255,255,255,0.85); }
      .vs-logo-handle-l { left: 0; } .vs-logo-handle-l::after { left: 3px; }
      .vs-logo-handle-r { right: 0; } .vs-logo-handle-r::after { right: 3px; }
      .vs-logo-layer-ico { color: #00d4ff; }
      .vs-logo-adv { margin: 2px 0 4px; border: 1px solid var(--line); border-radius: 10px; overflow: hidden; }
      .vs-logo-adv summary { cursor: pointer; padding: 9px 12px; font-size: 12px; font-weight: 700; color: var(--muted); list-style: none; user-select: none; display: flex; align-items: center; gap: 6px; }
      .vs-logo-adv summary::-webkit-details-marker { display: none; }
      .vs-logo-adv summary::before { content: "⚙"; font-size: 12px; }
      .vs-logo-adv[open] summary { border-bottom: 1px solid var(--line); color: var(--gold); }
      .vs-logo-adv > label { padding: 0 12px; }
      .vs-logo-adv > label:last-child { padding-bottom: 10px; }
      .vs-logo-uprow { display: flex; align-items: stretch; gap: 8px; }
      .vs-logo-remove { flex-shrink: 0; width: 40px; border-radius: 10px; border: 1px solid rgba(224,83,61,0.4); background: rgba(224,83,61,0.12); color: #e0533d; font-size: 15px; font-weight: 800; cursor: pointer; transition: all .15s; }
      .vs-logo-remove:hover { background: rgba(224,83,61,0.25); }
      .vs-autobrand-btn { width: 100%; margin: 4px 0 2px; padding: 11px; border-radius: 12px; cursor: pointer; font-size: 13px; font-weight: 800; color: #060606; background: linear-gradient(90deg, var(--gold), #f0d99a); border: none; display: flex; align-items: center; justify-content: center; gap: 8px; transition: transform .15s, box-shadow .15s; }
      .vs-autobrand-btn:hover { transform: translateY(-2px); box-shadow: 0 8px 22px rgba(216,183,106,0.3); }
      .vs-logo-tip { font-size: 11px; color: var(--muted); line-height: 1.5; margin: 2px 0 6px; }
...
<div class="vs-auto-actions">
              ...
              <div class="vstudio-block vtab-hidden" data-vtab-panel="logo">
                ...
                <div class="vs-logo-uprow">
                  <label class="upload-box small" id="vsLogoBox" style="flex:1;margin:0;">
                    <span id="vsLogoBoxTxt" data-i18n="vLogoHint">Click to upload logo — PNG (transparent) or animated WebM/MP4</span>
                    <input id="vsLogo" type="file" accept="image/*,video/*" />
                  </label>
                  <button id="vsLogoRemove" class="vs-logo-remove" type="button" title="Remove logo" style="display:none;">✕</button>
                </div>
                <label><span data-i18n="vLogoSampleLbl">Or try a sample logo</span>
                  <select id="vsLogoSample">
                    <option value="">— None —</option>
                    <option value="sample-logo-roomvu.webm">roomvu — full animated logo</option>
                    <option value="sample-logo-roomvu-cascade.webm">roomvu — letter cascade</option>
                  </select>
                </label>
                <button id="vsPlaceLogo" class="vs-autobrand-btn" type="button">
                  <span>＋</span> <span data-i18n="vPlaceLogoTxt">Place logo on video</span>
                </button>
                <p class="vs-logo-tip" id="vsLogoTip" data-i18n="vLogoTip">After uploading, tap "Place logo". Then drag the logo on the preview to move it, and drag the ends of its bar on the timeline to set when it appears.</p>
                ...
                <label><span data-i18n="vLogoShowLbl">Show logo on</span>
                  <select id="vsLogoShow">
                    <option value="all" selected>All slides</option>
                    <option value="last">Last slide only (outro)</option>
                    <option value="first">First slide only (intro)</option>
                  </select>
                </label>
                ...
                <details class="vs-logo-adv">
                  <summary id="vsLogoAdvLbl" data-i18n="vLogoAdvLbl">Advanced styling</summary>
              ...
              <div class="vs-timeline" id="vsTimeline">
                ...
                <div class="vs-scene-strip" id="vsSceneStrip">
                  <div class="vs-scene-scroll" id="vsSceneScroll">
                    <div class="vs-logo-layer" id="vsLogoLayer" style="display:none;">
                      <div class="vs-logo-layer-bar" id="vsLogoLayerBar">
                        <div class="vs-logo-handle vs-logo-handle-l" id="vsLogoHandleL"></div>
                        <span class="vs-logo-layer-ico">★</span>
                        <span id="vsLogoLayerTxt">Logo</span>
                        <div class="vs-logo-handle vs-logo-handle-r" id="vsLogoHandleR"></div>
                      </div>
                    </div>
      ...
      <div class="ld-nav">
        <div class="ld-nav-left">
          ...
          <div>
            ...
            <div class="ld-brand-sub" id="ldBrandSub">Real-Time Intelligence Center · v296</div>
          </div>
        </div>
    ...
    <script src="app.js?v=v296"></script>
