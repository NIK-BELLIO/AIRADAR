...
function vsAssembleFromSections(data, skipFootage) {
  ...
  sections.forEach((sec, i) => {
    ...
    if (sec.type === "infographic" && parsedStats.length >= 1) {
      ...
      const len = hl.length;
      let pool;
      if (len <= 34)       pool = ["bold-statement", "title-center", "quote", "title-left"];
      else if (len <= 64)  pool = ["title-center", "title-left", "quote", "caption"];
      else                 pool = ["caption", "title-center", "title-left"];   // long → roomy auto-fit
...
async function vsGenerateNarration(data, voice) {
  try {
    ...
    const url = "https://text.pollinations.ai/" + encodeURIComponent(script) +
...
function drawNewsBanner(ctx, W, H, elapsed, dsVal, vsOff, dsDur) {
  ...
  // ══════════════════════════════════════════════════════════
    ...
    ctx.shadowBlur=0;
    if(source){ctx.fillStyle="rgba(210,200,180,0.9)";ctx.font=`700 ${Math.round(U*0.032)}px Inter, sans-serif`;ctx.fillText(source,aX+aW*0.05,aY+lines.length*lH+U*0.06);}
