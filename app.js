...
// Strip website nav / section junk ("Home", "Local", "Subscriptions"…) from an
// extracted item list, prefer real "City, ST" entries when present, and cap it.
function vsCleanItemNames(names) {
  const STOP = new Set(["home","local","news","sports","finance","weather","entertainment",
    ...
    "faq","sources","methodology","mortgage","rent","taxes","savings","income","weather"]);
  ...
  // If several entries look like "City, ST" / "City, State", the article is a
  // place listicle — keep ONLY those and drop everything else (junk sections).
  const cityLike = out.filter(n => /,\s*[A-Z]{2}\b/.test(n) || /,\s*[A-Z][a-z]+/.test(n));
  if (cityLike.length >= 3) out = cityLike;
...
function vsBasicCityData(name, excerpt, topic, source, count) {
  ...
  const caps = ["SPOTLIGHT", "WHY IT MATTERS", "THE DETAILS", "WORTH KNOWING", "THE TAKEAWAY", "IN FOCUS", "BOTTOM LINE"];
  const sections = [];
  for (let i = 0; i < n; i++) {
    const headline = clip(sents[i], 90)
      || (i === 0 ? (short + " is worth a closer look.") : ("What makes " + short + " stand out."));
...
ARTICLE: """${String(text).slice(0, 12000)}"""`;
  ...
  names = vsCleanItemNames(names);   // drop nav/section junk, prefer cities, cap
  ...
  if (names.length < 2 && !vstudio._batchCancel) {
    vsAutoStatus(fa ? "متن کامل نبود — بازسازی فهرست با کمک مدل…"
                    : "Article was a teaser — reconstructing the list…");
    const numMatch = String(text).match(/\b(\d{1,2})\s+[A-Za-z]+\b/);
    const hintN = numMatch ? numMatch[1] : "";
    const kPrompt =
`The TEXT below is a news listicle (possibly only its intro/teaser; the full list may live on separate slides).
List EVERY item the article covers — cities, places, products, or people — in order, named exactly as such articles name them${hintN ? " (there should be about " + hintN + ")" : ""}.
If the TEXT doesn't spell them all out, use your knowledge of this widely-published article/topic to list them ALL accurately. Do NOT invent fictional entries.
Return ONLY compact JSON: {"topic":"overall subject in <=6 words","source":"outlet name if identifiable else a fitting label","items":["name 1","name 2", "..."]}
TEXT: """${String(text).slice(0, 8000)}"""`;
    try {
      const ex2 = vsParseAiJson(await vsAutoAiChat(kPrompt));
      const cleaned = vsCleanItemNames(namesFrom(ex2));
