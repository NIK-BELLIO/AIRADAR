...
async function vsAutoGenerateBackgrounds(data) {
  ...
  for (let i = 0; i < slides.length; i++) {
    if (vstudio._batchCancel) break;        // stop footage work on cancel
    const s = slides[i];
...
async function vsLoadBatchVideo(i) {
  ...
  const fa = state.lang === "fa";
  vsAssembleFromSections(v.data, true);          // build slides, footage below
  vsRenderBatchList();
  // footage is now ALWAYS auto-generated (real Pexels clips → AI image fallback)
  if (!v._footaged && !vstudio._batchCancel) {
...
async function vsExportAllBatch() {
  ...
  vstudio._batchExporting = false;
  vstudio.rendering = false;          // self-heal
  ...
  vsAutoStatus(wasCancelled
    ...
    : (fa ? `همهٔ ${vids.length} ویدئو دانلود شد.` : `All ${vids.length} videos downloaded.`));
}
