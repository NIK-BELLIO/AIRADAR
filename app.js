...
try { console.log("%cAI Radar Studio build v341", "color:#d8b76a;font-weight:bold"); } catch(e){}
...
async function vsLoadBatchVideo(i) {
  ...
  // Background music (default ON) — calm cinematic bed, ducked under narration.
  ...
  // AI voiceover (free Pollinations TTS) — generate once per video, cache it,
  // and set it as the narration track so preview + export include it.
  ...
  if (voTog && voTog.checked) {
    if (!v._narrationEl && !vstudio._batchCancel) {
      ...
      if (nar) { v._narrationEl = nar.el; v._narrationUrl = nar.url; }
    }
