...
async function vsGenerateAiMusic(data) {
  if (!VS_POLLINATIONS_KEY) return null;
  ...
  try {
    ...
    try {
      resp = await fetch("https://gen.pollinations.ai/v1/audio/speech", {
        ...
        headers: { "Content-Type": "application/json", "Authorization": "Bearer " + VS_POLLINATIONS_KEY },
...
async function vsGenerateNarration(data, voice) {
  ...
  if (!blob) return vsGenerateNarrationLegacy(data, voice);
