...
function startHeroBackground() {
  ...
  function buildNodes() {
    const count = Math.max(26, Math.min(88, Math.round(W / 20)));
  ...
  function draw() {
    ...
    blobs.forEach(b => {
      ...
      rg.addColorStop(0, b.c + b.a + ")"); rg.addColorStop(1, b.c + "0)");
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      for (let j = i + 1; j < nodes.length; j++) {
        ...
        if (dist < D) {
          ctx.strokeStyle = "rgba(216,183,106," + ((1 - dist / D) * 0.26).toFixed(3) + ")";
